...
import { t, setLanguage, getLanguages, getCurrentLanguage, isRTL, currentLang, getTranslatedLanguages } from './i18n';
import { theme, setTheme, isDark, activeTheme } from './theme';
import { departments, getDepartment, getDepartmentSkills, getSkill, getSkillGroups, type Department, type SkillGroup } from './departments';
...
// ============== MY BOARD — pinned departments ==============
// ============== MY BOARD — smart personal panel ==============
type BoardItem = { id: string; type: 'skill' | 'dept'; source: 'auto' | 'manual'; active: boolean; role?: 'worker' | 'client' | 'course' };
...
const [boardItems, setBoardItems] = createSignal<BoardItem[]>(_migrateBoard());
createEffect(() => { try { localStorage.setItem(BOARD_KEY, JSON.stringify(boardItems())); } catch {} });

// Board helpers
const boardHas = (id: string) => boardItems().some(b => b.id === id);
const boardGet = (id: string) => boardItems().find(b => b.id === id);
const boardIsActive = (id: string) => boardGet(id)?.active ?? false;

const boardAdd = (id: string, type: 'skill' | 'dept', source: 'auto' | 'manual', role: 'worker' | 'client' | 'course' = 'worker') => {
  if (!boardHas(id)) setBoardItems(prev => [...prev, { id, type, source, active: true, role }]);
};
const boardRemove = (id: string) => {
  setBoardItems(prev => prev.filter(b => b.id !== id));
};
const boardToggleActive = (id: string) => {
  setBoardItems(prev => prev.map(b => b.id === id ? { ...b, active: !b.active } : b));
};
const boardSetAllActive = (active: boolean) => {
...
createEffect(() => {
  ...
  setBoardItems(prev => {
    let next = [...prev];
    // Add new workerSkills as auto
    for (const sid of ws) {
      if (!next.some(b => b.id === sid)) next.push({ id: sid, type: 'skill', source: 'auto', active: true, role: 'worker' });
    }
    // Add new clientNeeds as auto
    for (const sid of cn) {
      if (!next.some(b => b.id === sid)) next.push({ id: sid, type: 'skill', source: 'auto', active: true, role: 'client' });
    }
    // Remove auto items that are no longer in workerSkills/clientNeeds
    next = next.filter(b => {
      if (b.source !== 'auto') return true;
      if (b.role === 'worker') return ws.includes(b.id);
      if (b.role === 'client') return cn.includes(b.id);
      return true; // courses stay
    });
    return next;
  });
});

// Legacy compat wrappers (used by profile compact summary)
const pinnedDepts = () => boardItems().filter(b => b.type === 'dept').map(b => b.id);
const pinnedSkills = () => boardItems().filter(b => b.type === 'skill').map(b => b.id);
const togglePin = (deptId: string) => boardHas(deptId) ? boardRemove(deptId) : boardAdd(deptId, 'dept', 'manual');
const togglePinSkill = (skillId: string) => boardHas(skillId) ? boardRemove(skillId) : boardAdd(skillId, 'skill', 'manual');

// Likes — shared state from store
import { initLikes, getLikeCount, hasLiked, likeOnce } from './store';
...
function LikeBadge(props: { likeKey: string; compact?: boolean }) {
  ...
  return (
    <button
      type="button"
      class={`absolute ${compact() ? 'top-0 right-0 px-[3px] py-[1px] rounded-bl-md' : 'top-0 right-0 px-1 py-0.5 rounded-bl-lg'} flex items-center gap-[2px] z-10 transition-all`}
      style={`backdrop-filter: blur(6px); -webkit-backdrop-filter: blur(6px); background: rgba(0,0,0,0.25);`}
      onClick={(e) => {
        e.preventDefault();
        e.stopPropagation();
    ...
    >
      <Icon name="heart" size="xs" class={liked() ? 'text-white' : 'text-white/60'} />
      <Show when={count() > 0}>
        <span class={`${compact() ? 'text-[7px]' : 'text-[8px]'} font-semibold text-white/80`}>{count()}</span>
      </Show>
    </button>
  );
}

// UI components & utilities — imported from ui.tsx (single source of truth)
import { Icons, Icon, EMOJI_TO_ICON, SkillIcon, SwipeLayer, SwipeBack } from './ui';
import { playGlobalSound, haptic, hapticOrder } from './ui';
import { globalSoundEnabled, setGlobalSoundEnabled, globalHapticEnabled, setGlobalHapticEnabled } from './ui';
import { globalNotifSound, setGlobalNotifSound, globalVolume, setGlobalVolume } from './ui';
import { vibrationIntensity, setVibrationIntensity, rareEscalationEnabled, setRareEscalationEnabled } from './ui';
...
const ELINA_COLORS: Record<string, { base: string; mid: string; light: string; deep: string; glow: string }> = {
  deepBlue:   { base: '#0a1628', mid: '#152d50', light: '#1e6cb5', deep: '#050d1a', glow: 'rgba(30,108,181,0.4)' },
  ...
  steel:      { base: '#1a1f2e', mid: '#2d3a52', light: '#5a7baa', deep: '#0e1320', glow: 'rgba(90,123,170,0.35)' },
};
...
function MobileElina(props: { size?: number }) {
  ...
  const [colorKey, setColorKey] = createSignal('deepBlue');
  ...
  const c = () => ELINA_COLORS[colorKey()] || ELINA_COLORS.deepBlue;

  // Fire particles — realistic upward movement
  const spawnFire = (n: number) => {
    const fireColors = ['#1e6cb5', '#2196F3', '#42a5f5', '#64b5f6', '#90caf9', '#bbdefb', '#e3f2fd', '#4fc3f7', '#0288d1'];
  ...
  const glowShadow = () => {
    const g = c().glow;
    if (eState() === 'secret') return `0 0 ${sz()}px ${g}, 0 0 ${sz() * 2}px rgba(30,108,181,0.5), 0 0 ${sz() * 3}px rgba(33,150,243,0.25)`;
    ...
    >
      ...
      <Show when={screenFlash()}>
        <div class="fixed inset-0 pointer-events-none z-[9999] el3-flash" style={{
          background: 'radial-gradient(circle at 50% 40%, rgba(30,108,181,0.5) 0%, rgba(33,150,243,0.25) 40%, transparent 70%)',
        }} />
      </Show>
      ...
      <Show when={flames()}>
        <div class="absolute pointer-events-none el3-flame-container" style={{
          ...
          <div class="el3-flame el3-flame-1" style={{ background: 'linear-gradient(to top, #0a3d7a, #1565c0, #42a5f5, transparent)', width: `${sz() * 0.5}px`, height: `${sz() * 1.5}px` }} />
          <div class="el3-flame el3-flame-2" style={{ background: 'linear-gradient(to top, #0d47a1, #1976d2, #64b5f6, transparent)', width: `${sz() * 0.35}px`, height: `${sz() * 1.2}px` }} />
          <div class="el3-flame el3-flame-3" style={{ background: 'linear-gradient(to top, #1565c0, #42a5f5, #90caf9, transparent)', width: `${sz() * 0.25}px`, height: `${sz() * 1.0}px` }} />
        </div>
      </Show>
      ...
      <div class={`cursor-pointer relative ${animCls()}`} style={{
        ...
        <div class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none" style={{ 'z-index': '5' }}>
          {eState() === 'secret'
            ? <span style={{ 'font-size': `${sz() * 0.4}px`, filter: 'drop-shadow(0 0 8px rgba(30,108,181,0.9)) drop-shadow(0 0 16px rgba(33,150,243,0.5))' }}>{'\u{26A1}'}</span>
            : <>
              {/* Eyes */}
...
function HomePage(props: { onNavigate: (page: string) => void }) {
  // Notification badge count (mock for now)
  const unreadCount = () => 3;
  ...
  const [elinaMsgs, setElinaMsgs] = createSignal<{ from: 'user' | 'elina'; text: string }[]>([]);
  const [elinaInput, setElinaInput] = createSignal('');
  const [elinaTyping, setElinaTyping] = createSignal(false);
  let chatEndRef: HTMLDivElement | undefined;

  const isRu = () => currentLang() === 'ru' || currentLang() === 'kk' || currentLang() === 'uk';

  const elinaKnowledge = () => isRu() ? [
    { keys: ['привет', 'здравствуй', 'хай', 'салам'], answer: 'Привет! Я Элина — твой помощник в BOLH. Спроси меня о любой функции приложения или о том, что можно улучшить!' },
    { keys: ['что ты умеешь', 'что можешь', 'помощь'], answer: 'Я знаю всё о BOLH! Могу рассказать про: заказы, карту, кошелёк, контракты, академию, борд, профиль, безопасность, документы и маркетплейс.' },
    { keys: ['заказ', 'срочный', 'urgent'], answer: 'Срочный заказ — мастер за 15 минут! Нажми "Срочный заказ" на главной. Мастера в радиусе 3 км получат уведомление и предложат свою цену.' },
    { keys: ['карта', 'map', 'рядом'], answer: 'На карте видно всех мастеров рядом в реальном времени. Можно фильтровать по департаментам и сразу заказать.' },
    { keys: ['кошелёк', 'wallet', 'блокчейн', 'оплата'], answer: 'В кошельке — баланс, история транзакций и блокчейн-интеграция (скоро). Поддерживаем карты, BOLH-баланс и мгновенные переводы.' },
    { keys: ['контракт', 'эскроу', 'escrow'], answer: 'Контракты: мгновенные (1-24ч), короткие (1-7 дней), месячные и подписки. Деньги защищены эскроу — мастер получит оплату только после завершения.' },
    { keys: ['академия', 'обучение'], answer: 'BOLH Академия — 16 модулей: профессиональные стандарты, безопасность, первая помощь. По ISO, AHA, OSHA стандартам. Зарабатывай очки!' },
    { keys: ['борд', 'панель', 'навыки'], answer: 'Мой Борд — твоя визитная карточка. Добавляй навыки, активируй/деактивируй их. Есть режим "Мастер" и "Клиент".' },
    { keys: ['профиль', 'настройки'], answer: 'В профиле: заказы, рейтинг, стаж, документы, академия, безопасность, верификация, язык и тема.' },
    { keys: ['безопасность', 'пин'], answer: 'Центр безопасности: PIN, биометрия, 2FA, авто-блокировка. Данные зашифрованы AES-256. Экстренные контакты и журнал активности.' },
    { keys: ['документ'], answer: 'Хранилище документов — AES-256. Загружай дипломы, сертификаты, паспорта. Делись по ссылке с таймером или PIN.' },
    { keys: ['маркетплейс', 'магазин'], answer: 'Маркетплейс — оборудование, униформа, курсы и средства безопасности для профессионалов.' },
    { keys: ['идея', 'развитие', 'улучшить', 'фича'], answer: 'Идеи для BOLH:\n• AI-подбор мастеров по рейтингу\n• Видео-звонки до заказа\n• Кэшбек и лояльность\n• Страхование заказов\n• Групповые заказы\n• Реферальная программа\n• Интеграция с умным домом\n• AR-просмотр на карте' },
    { keys: ['язык', 'перевод'], answer: 'BOLH поддерживает 60+ языков! Переключи в Профиль → Язык.' },
    { keys: ['тема', 'тёмная', 'светлая'], answer: 'Есть тёмная и светлая темы. Переключи в Профиль → Тема.' },
    { keys: ['рейтинг', 'отзыв'], answer: 'После заказа оставь отзыв: 1-5 звёзд + теги + комментарий.' },
  ] : [
    { keys: ['hello', 'hi', 'hey', 'sup'], answer: 'Hey! I\'m Elina — your BOLH assistant. Ask me about any app feature or ideas for development!' },
    { keys: ['what can you', 'help', 'features'], answer: 'I know everything about BOLH! Ask about: orders, map, wallet, contracts, academy, board, profile, security, documents, marketplace.' },
    { keys: ['order', 'urgent'], answer: 'Urgent Order — worker in 15 minutes! Workers within 3 km get notified and can accept your price or offer their own.' },
    { keys: ['map', 'nearby'], answer: 'Map shows all workers near you in real-time. Filter by department and order directly.' },
    { keys: ['wallet', 'blockchain', 'payment'], answer: 'Wallet: balance, transactions, blockchain integration (coming soon). Cards, BOLH balance, instant transfers.' },
    { keys: ['contract', 'escrow'], answer: 'Contracts: instant (1-24h), short (1-7 days), monthly, subscriptions. Escrow protects money until work is done.' },
    { keys: ['academy', 'training'], answer: 'BOLH Academy — 16 modules: pro standards, safety, first aid. ISO/AHA/OSHA compliant. Earn points and level up!' },
    { keys: ['board', 'skills'], answer: 'My Board is your business card. Add skills, toggle active/inactive. Switch Worker/Client modes.' },
    { keys: ['profile', 'settings'], answer: 'Profile: orders, rating, experience, documents, academy, security, verification, language, theme.' },
    { keys: ['security', 'pin'], answer: 'Security Center: PIN, biometrics, 2FA, auto-lock. AES-256 encryption. Emergency contacts and activity log.' },
    { keys: ['document', 'vault'], answer: 'Document Vault — AES-256 encrypted. Upload diplomas, certificates, IDs. Share via timed link or PIN.' },
    { keys: ['marketplace', 'shop'], answer: 'Marketplace — equipment, uniforms, training, safety gear for professionals.' },
    { keys: ['idea', 'improve', 'feature', 'develop'], answer: 'Ideas for BOLH:\n• AI worker matching\n• Video calls before ordering\n• Loyalty cashback\n• Order insurance\n• Group orders\n• Referral bonuses\n• Smart home integration\n• AR map preview' },
    { keys: ['language'], answer: 'BOLH supports 60+ languages! Switch in Profile → Language.' },
    { keys: ['theme', 'dark', 'light'], answer: 'Dark and Light themes. Switch in Profile → Theme.' },
    { keys: ['rating', 'review'], answer: 'After each order: 1-5 stars + tags + text review.' },
  ];

  const getElinaReply = (msg: string): string => {
    const lower = msg.toLowerCase();
    for (const item of elinaKnowledge()) {
      if (item.keys.some(k => lower.includes(k))) return item.answer;
    }
    return isRu()
      ? 'Попробуй спросить про: заказы, карту, кошелёк, контракты, академию, борд, безопасность, документы, маркетплейс или идеи развития.'
      : 'Try asking about: orders, map, wallet, contracts, academy, board, security, documents, marketplace, or development ideas.';
  };

  const sendElinaMsg = () => {
    const text = elinaInput().trim();
    if (!text) return;
    setElinaMsgs(m => [...m, { from: 'user', text }]);
    setElinaInput('');
    setElinaTyping(true);
    haptic('light');
    setTimeout(() => chatEndRef?.scrollIntoView({ behavior: 'smooth' }), 50);
    setTimeout(() => {
      setElinaTyping(false);
      setElinaMsgs(m => [...m, { from: 'elina', text: getElinaReply(text) }]);
      setTimeout(() => chatEndRef?.scrollIntoView({ behavior: 'smooth' }), 50);
    }, 400 + Math.random() * 800);
  };

  const openElinaChat = () => {
    setElinaChatOpen(true);
    haptic('light');
    if (elinaMsgs().length === 0) {
      setElinaMsgs([{ from: 'elina', text: isRu()
        ? 'Привет! Я Элина, помощник BOLH. Спроси о чём угодно — функциях, заказах, идеях!'
        : 'Hi! I\'m Elina, your BOLH assistant. Ask me anything — features, orders, ideas!' }]);
    }
  };
  ...
  return (
    <div class="p-4 animate-fade-in">
      ...
      <div class="flex items-center gap-3 mb-5">
        <div class="shrink-0 touch-scale cursor-pointer" onClick={openElinaChat}>
          <MobileElina size={48} />
        </div>
        <Show when={!elinaChatOpen()}>
          <div class="flex-1" onClick={openElinaChat}>
            <p class="text-white/30 text-[10px] font-medium uppercase tracking-wider">{isRu() ? 'Нажми — спроси Элину' : 'Tap — ask Elina'}</p>
          </div>
        </Show>
      </div>
      ...
      {/* Elina Chat Panel */}
      <Show when={elinaChatOpen()}>
        <div class="mb-4 rounded-2xl overflow-hidden border border-white/[0.08] animate-slide-up" style="background: rgba(10,22,40,0.85); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px);">
          <div class="flex items-center justify-between px-4 py-2.5 border-b border-white/[0.06]">
            <div class="flex items-center gap-2">
              <div class="w-2 h-2 rounded-full bg-sky-400 animate-pulse" />
              <span class="text-white/80 text-sm font-semibold">Elina</span>
              <span class="text-white/30 text-xs">{isRu() ? 'онлайн' : 'online'}</span>
            </div>
            <button class="text-white/30 hover:text-white/60 p-1 transition-colors" onClick={() => setElinaChatOpen(false)}>
              <Icon name="x" size="sm" />
            </button>
          </div>
          <div class="max-h-[280px] overflow-y-auto p-3 space-y-2.5" style="scrollbar-width: none;">
            <For each={elinaMsgs()}>
              {(msg) => (
                <div class={`flex ${msg.from === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div class={`max-w-[85%] px-3 py-2 rounded-2xl text-sm leading-relaxed whitespace-pre-line ${
                    msg.from === 'user'
                      ? 'bg-sky-600/40 text-white/90 rounded-br-md'
                      : 'bg-white/[0.08] text-white/80 rounded-bl-md'
                  }`}>{msg.text}</div>
                </div>
              )}
            </For>
            <Show when={elinaTyping()}>
              <div class="flex justify-start">
                <div class="bg-white/[0.08] px-4 py-2.5 rounded-2xl rounded-bl-md flex gap-1">
                  <div class="w-1.5 h-1.5 rounded-full bg-white/40 animate-bounce" style="animation-delay: 0ms" />
                  <div class="w-1.5 h-1.5 rounded-full bg-white/40 animate-bounce" style="animation-delay: 150ms" />
                  <div class="w-1.5 h-1.5 rounded-full bg-white/40 animate-bounce" style="animation-delay: 300ms" />
                </div>
              </div>
            </Show>
            <div ref={chatEndRef} />
          </div>
          <Show when={elinaMsgs().length <= 1}>
            <div class="px-3 pb-2 flex flex-wrap gap-1.5">
              <For each={isRu() ? ['Заказы', 'Карта', 'Контракты', 'Идеи развития', 'Академия', 'Безопасность'] : ['Orders', 'Map', 'Contracts', 'Dev ideas', 'Academy', 'Security']}>
                {(tag) => (
                  <button class="px-2.5 py-1 rounded-full text-[11px] font-medium bg-white/[0.06] text-white/50 hover:bg-white/[0.12] hover:text-white/70 transition-all touch-scale"
                    onClick={() => { setElinaInput(tag); setTimeout(sendElinaMsg, 50); }}>{tag}</button>
                )}
              </For>
            </div>
          </Show>
          <div class="flex items-center gap-2 p-2.5 border-t border-white/[0.06]">
            <input type="text" class="flex-1 bg-white/[0.06] text-white/90 text-sm rounded-xl px-3 py-2 outline-none placeholder:text-white/25 focus:bg-white/[0.1] transition-colors"
              placeholder={isRu() ? 'Спроси о BOLH...' : 'Ask about BOLH...'}
              value={elinaInput()} onInput={(e) => setElinaInput(e.currentTarget.value)}
              onKeyDown={(e) => { if (e.key === 'Enter') sendElinaMsg(); }} />
            <button class="w-9 h-9 rounded-xl bg-sky-600/50 hover:bg-sky-600/70 flex items-center justify-center transition-all touch-scale"
              onClick={sendElinaMsg}>
              <Icon name="arrowRight" size="sm" class="text-white" />
            </button>
          </div>
        </div>
      </Show>
      {/* Ad / Promo Banner — rotating */}
      {(() => {
        const [adIdx, setAdIdx] = createSignal(0);
        const ads = [
          { text: currentLang() === 'en' ? 'Need a pro fast? Try Urgent Order!' : 'Нужен мастер срочно? Попробуй Срочный заказ!', color: '#0ea5e9', lucideIcon: 'zap', action: 'urgent' },
          { text: currentLang() === 'en' ? 'Earn more — add your skills to My Board' : 'Зарабатывай больше — добавь навыки в Моя панель', color: '#0ea5e9', lucideIcon: 'layers', action: 'myboard' },
          { text: currentLang() === 'en' ? 'Invite friends — get bonuses!' : 'Пригласи друзей — получи бонусы!', color: '#10b981', lucideIcon: 'userPlus', action: 'referral' },
          { text: currentLang() === 'en' ? 'Your security — our priority. Verify now' : 'Твоя безопасность — наш приоритет. Верифицируйся', color: '#22c55e', lucideIcon: 'shieldCheck', action: 'verification' },
          { text: currentLang() === 'en' ? 'Find the perfect pro — Search & Filter!' : 'Найди идеального мастера — Поиск и фильтры!', color: '#3b82f6', lucideIcon: 'search', action: 'discover' },
        ];
        ...
        return (
          <div
            class="flex items-center gap-3 p-3 rounded-2xl mb-4 cursor-pointer touch-scale animate-slide-up"
            style={`background: ${ad().color}15; border: 1px solid ${ad().color}25; animation-delay: 0.05s;`}
            onClick={() => props.onNavigate(ad().action)}
          >
            <Icon name={ad().lucideIcon} size="sm" class="text-white/90 shrink-0" />
            <p class="text-white/90 text-xs font-semibold flex-1 leading-snug">{ad().text}</p>
            <span class="text-[9px] px-1.5 py-0.5 rounded-md font-bold" style={`color: ${ad().color}; background: ${ad().color}12;`}>AD</span>
          </div>
        );
      })()}
              ...
              {(dept, i) => {
                ...
                return (
                  <button
                    ...
                    style={`animation-delay: ${0.1 + i() * 0.03}s; background: rgba(255,255,255,0.06); border: 2px solid rgba(255,255,255,0.14);`}
                  ...
                  <div class="h-full flex flex-col p-3">
                    ...
                    <div class="flex items-center gap-3 mb-3 shrink-0">
                      ...
                      <Show when={activeGroup() || activeSkillWithVariants()} fallback={
                        <div class={`w-10 h-10 rounded-xl bg-gradient-to-br ${dept()?.color || 'from-sky-600 to-blue-700'} flex items-center justify-center shadow-lg shrink-0`}>
                              ...
                              return (
                                ...
                                >
                                  <div class={`relative w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 ${
                                    grpSelected() > 0
                                      ? 'bg-gradient-to-br ' + (dept()?.color || 'from-sky-600 to-blue-700') + ' shadow-lg'
                                      : 'bg-white/10'
                                  }`}>
                            ...
                            return (
                              ...
                              >
                                <div class={`relative w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${
                                  sel()
                                    ? 'bg-gradient-to-br ' + (dept()?.color || 'from-sky-600 to-blue-700') + ' shadow'
                                    : 'bg-white/10'
                                }`}>
      ...
      {/* Active Order Card — will appear dynamically when a real order is in progress */}
    </div>
  );
}
...
function DepartmentViewPage(props: { onNavigate: (page: string) => void; onBack: () => void }) {
  ...
  return (
    <div class="animate-fade-in">
      {/* Department Header */}
      <div class={`bg-gradient-to-br ${dept()?.color || 'from-sky-600 to-blue-700'} p-5 pb-6`}>
            ...
            {(skill) => (
              <span class="px-3 py-1.5 bg-white/20 backdrop-blur rounded-full text-white text-xs font-medium whitespace-nowrap flex items-center gap-1">
                ...
                <Show when={skill.requiresDiploma}>
                  <Icon name="graduationCap" size="xs" class="text-sky-500 inline-block" />
                </Show>
              </span>
            )}
          </For>
          ...
          {[
            ...
            <button
              class={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-all ${
                filter() === f.id
                  ? 'bg-white/90 shadow-sm ' + (dept()?.accentText || 'text-sky-600')
                  : 'glass text-gray-600'
              }`}
            ...
            {(worker, i) => (
              ...
              >
                <div class="flex items-start gap-4">
                  <div class="relative">
                    <div class="w-16 h-16 rounded-2xl bg-gradient-to-br from-slate-200 to-slate-300 flex items-center justify-center">
                      <Icon name="user" size="lg" class="text-slate-500" />
                    </div>
                    {worker.online && (
                  ...
                  <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2">
                      ...
                      {worker.verified && (
                        <div class="w-5 h-5 bg-sky-500 rounded-full flex items-center justify-center flex-shrink-0">
                          <Icon name="check" class="text-white w-3 h-3" />
                        </div>
                      )}
                    </div>
                    <p class={`text-xs font-medium mt-0.5 ${dept()?.accentText || 'text-sky-600'}`}>
                  ...
                  <div class="text-right flex-shrink-0">
                    <p class="text-lg font-bold" style={`color: ${dept()?.colorFrom || '#0ea5e9'}`}>{worker.price.toLocaleString()}</p>
                    <p class="text-xs text-gray-400">{t('search.perHour')}</p>
                  </div>
                </div>

                <button 
                  class={`w-full mt-4 py-3 bg-gradient-to-r ${dept()?.color || 'from-sky-600 to-blue-700'} text-white rounded-2xl font-semibold shadow-lg touch-scale`}
                >
                  {t('search.order')}
                </button>
              </div>
...
function WorkerSkillsPage(props: { onBack: () => void }) {
  ...
  const statusIconName = () => {
    const s = workerStatus();
    if (s === 'online') return 'checkCircle';
  ...
  return (
    <div class="animate-fade-in">
      {/* Header */}
      <div class="bg-gradient-to-br from-sky-600 to-blue-700 p-5 pb-6">
        <div class="flex items-center gap-3 mb-4">
          <button class="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center touch-scale" onClick={props.onBack}>
            <Icon name="chevronLeft" class="text-white" size="sm" />
          </button>
        ...
        <div class="glass rounded-2xl p-4" style="background: rgba(255,255,255,0.15)">
          <div class="flex items-center gap-3 mb-3">
            <div class={`w-12 h-12 rounded-xl bg-gradient-to-br ${statusColor()} flex items-center justify-center shadow-md`}>
              <Icon name={statusIconName()} size="sm" class="text-white" />
            </div>
            <div class="flex-1">
                        ...
                        {(skill) => {
                          ...
                          return (
                            ...
                            >
                              <div class={`relative w-14 h-14 rounded-2xl flex items-center justify-center ${
                                active()
                                  ? 'bg-gradient-to-br ' + (dept()?.color || 'from-sky-600 to-blue-700')
                                  : isLocked
                                  ? 'bg-gray-200'
                                  : 'bg-gray-100'
                              }`}>
          ...
          <div class="glass rounded-3xl p-6 max-w-sm w-full animate-slide-up" onClick={(e) => e.stopPropagation()}>
            <div class="text-center mb-5">
              <div class="w-16 h-16 rounded-full bg-slate-100 dark:bg-slate-800/60 flex items-center justify-center mx-auto mb-3">
                <Icon name="graduationCap" size="xl" class="text-sky-500" />
              </div>
            ...
            <button
              class="w-full py-3 bg-gradient-to-r from-blue-500 to-sky-600 text-white rounded-2xl font-semibold mb-3 touch-scale flex items-center justify-center gap-2"
          ...
          <div class="p-4 space-y-5">
            ...
            <div class="glass rounded-3xl p-5">
              <div class="flex items-center justify-between mb-3">
                ...
                <span class="text-xs text-sky-600 font-medium flex items-center gap-0.5"><Icon name="location" size="xs" class="text-sky-600" /> GPS</span>
              </div>
              <div class="flex items-center gap-3 p-3 bg-gray-50 rounded-2xl">
                <div class="w-10 h-10 rounded-full bg-sky-100 flex items-center justify-center">
                  <Icon name="location" class="text-sky-600" size="sm" />
                </div>
                <input 
                  type="text" 
                  value={address()}
            ...
            <div class="glass rounded-3xl p-5">
              ...
              <div class="relative">
                <input 
                  ...
                  class="w-full h-2 bg-gray-200 rounded-full appearance-none cursor-pointer accent-sky-600"
                />
                <div class="flex justify-between text-xs text-gray-400 mt-3">
                  <span>5 000 ₸</span>
                  <span class="text-sky-500 font-medium">{t('urgent.recommend')}</span>
                  <span>50 000 ₸</span>
                </div>
              </div>
            </div>
            ...
            {/* Continue Button */}
            <button 
              class="w-full py-4 bg-gradient-to-r from-sky-600 to-blue-700 text-white rounded-2xl font-bold text-lg shadow-xl touch-scale"
        ...
        <Match when={step() === 'confirm'}>
          <div class="p-4 space-y-5">
            ...
            <div class="glass rounded-3xl overflow-hidden">
              <div class="p-5 bg-gradient-to-br from-sky-600 to-blue-700 text-white">
            ...
            <div class="glass rounded-3xl p-5">
              ...
              <div class="space-y-3">
                <div class="flex items-start gap-3">
                  <div class="w-7 h-7 rounded-full bg-sky-100 flex items-center justify-center flex-shrink-0 text-sm font-bold text-sky-600">1</div>
                  <p class="text-sm text-gray-600">{t('urgent.step1')}</p>
                </div>
                <div class="flex items-start gap-3">
                  <div class="w-7 h-7 rounded-full bg-sky-100 flex items-center justify-center flex-shrink-0 text-sm font-bold text-sky-600">2</div>
                ...
                <div class="flex items-start gap-3">
                  <div class="w-7 h-7 rounded-full bg-sky-100 flex items-center justify-center flex-shrink-0 text-sm font-bold text-sky-600">3</div>
          ...
          <div class="p-4">
            ...
            <div class="flex gap-4 mb-6">
              <div class="flex-1 glass rounded-2xl p-4 text-center">
                <div class="text-3xl font-bold text-sky-600">{countdown()}</div>
            ...
            <Show when={offers().length >= 2}>
              <button 
                class="w-full mt-6 py-3 glass rounded-2xl text-sky-600 font-medium touch-scale"
        ...
        <Match when={step() === 'offers'}>
          <div class="p-4">
            ...
            <div class="glass rounded-2xl p-4 mb-4">
              <div class="flex items-center justify-between">
                ...
                <div class="text-right">
                  ...
                  <p class="font-bold text-sky-600">{budget().toLocaleString()} ₸</p>
                </div>
              </div>
            </div>
            ...
            <div class="flex gap-2 mb-4 overflow-x-auto pb-1">
              <button class="px-4 py-2 bg-sky-600 text-white rounded-full text-sm font-medium whitespace-nowrap">
        ...
        <Match when={step() === 'selected'}>
          <Show when={selectedOffer()}>
            <div class="p-4 space-y-5">
              {/* Guard card */}
              <div class="glass rounded-3xl overflow-hidden">
                <div class="p-6 bg-gradient-to-br from-sky-600 to-blue-700 text-white text-center">
                ...
                <div class="p-5">
                  ...
                  <div class="space-y-3">
                    ...
                    <div class="border-t border-gray-100 pt-3 flex justify-between">
                      ...
                      <span class="text-2xl font-bold text-sky-600">{selectedOffer().price.toLocaleString()} ₸</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div class="w-full glass rounded-3xl p-5 mb-6">
              ...
              <div class="flex gap-3">
                <button class="flex-1 py-3 glass rounded-xl flex items-center justify-center gap-2 touch-scale">
                  <Icon name="phone" class="text-sky-600" size="sm" />
                  <span class="font-medium text-gray-700">{t('tracking.call')}</span>
                </button>
                <button class="flex-1 py-3 bg-gradient-to-r from-sky-600 to-blue-700 rounded-xl flex items-center justify-center gap-2 shadow-lg touch-scale">
    ...
    <div class="p-4 animate-fade-in">
      ...
      <div class="flex gap-2 mb-4 overflow-x-auto pb-1">
        <button
          class={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-all ${
            !deptFilter() ? 'bg-white/90 text-sky-600 shadow-sm' : 'glass text-gray-500'
          }`}
        ...
        <For each={departments}>
          {(dept) => (
            ...
            >
              <SkillIcon icon={dept.icon} size="xs" />
      ...
      <div class="flex gap-2 mb-5 overflow-x-auto pb-1">
        {[
          ...
          <button
            class={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-all ${
              searchFilter() === f.id ? 'bg-white/90 text-sky-600 shadow-sm' : 'glass text-gray-600'
            }`}
            ...
            return (
              ...
              >
                <div class="flex items-start gap-4">
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <Show when={gd()}>
                      <span class={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium mt-1 ${gd()!.accentBg} ${gd()!.accentText}`}>
                        <SkillIcon icon={gd()!.icon} size="xs" />
                  ...
                  <div class="text-right flex-shrink-0">
                    <p class="text-lg font-bold" style={`color: ${gd()?.colorFrom || '#0ea5e9'}`}>{guard.price.toLocaleString()}</p>
                    <p class="text-xs text-gray-400">{t('search.perHour')}</p>
                  </div>
                </div>

                <button class={`w-full mt-4 py-3 bg-gradient-to-r ${gd()?.color || 'from-sky-600 to-blue-700'} text-white rounded-2xl font-semibold shadow-lg touch-scale`}>
                  {t('search.order')}
                </button>
              </div>
            );
...
function MapPage() {
  ...
  onMount(() => {
    ...
    tileLayer = L.tileLayer(url, { attribution: 'В© OSM', maxZoom: 19 }).addTo(map);
    setMapRef(map);
  ...
  createEffect(() => {
    ...
    list.forEach((w) => {
      ...
      const colorFrom = dept?.colorFrom ?? '#334155';
      const colorTo = dept?.colorTo ?? '#1e293b';
      const iconName = EMOJI_TO_ICON[dept?.icon ?? ''] ?? 'user';
      const icon = L.divIcon({
        className: 'map-worker-marker',
        html: `<div class="map-worker-circle" style="background: linear-gradient(135deg, ${colorFrom}, ${colorTo}); box-shadow: 0 4px 20px ${colorFrom}40;"><svg viewBox="0 0 256 256" fill="white" width="22" height="22" style="display:block;">${(Icons as any)[iconName]?.().props?.children?.props?.d ? '<path d="' + (Icons as any)[iconName]().props.children.props.d + '"/>' : ''}</svg></div>`,
        iconSize: [44, 44],
        iconAnchor: [22, 22],
      });
      const statusText = w.status === 'available' ? t('map.available') : t('map.busy');
      const popupContent = `<div class="map-popup"><strong>${w.name}</strong><br/><span class="text-gray-600">${w.profession}</span><br/><span class="text-amber-600">в… ${w.rating}</span> (${w.reviews}) В· ${w.distance} km ${t('map.fromYou')}<br/><span class="text-xs text-gray-500">${statusText}</span></div>`;
  ...
  createEffect(() => {
    ...
    tileLayer = L.tileLayer(url, { attribution: 'В© OSM', maxZoom: 19 }).addTo(m);
  });
  ...
  return (
    <div class="h-screen relative animate-fade-in overflow-hidden">
          ...
          <div class="flex gap-1.5 overflow-x-auto max-w-[50%]">
            <button
              class={`px-2.5 py-1 rounded-full text-xs font-medium whitespace-nowrap transition-all ${
                !departmentFilter()
                  ? 'bg-slate-700 dark:bg-white text-white dark:text-slate-800 shadow-md'
      ...
      <button
        type="button"
        class="absolute bottom-36 right-4 z-10 w-12 h-12 rounded-2xl shadow-xl border border-white/20 backdrop-blur-xl bg-white/80 dark:bg-gray-900/80 flex items-center justify-center text-slate-700 dark:text-slate-300 touch-scale"
                    ...
                    <div class="p-3">
                      <div class="flex items-center gap-2 mb-2">
                        <div
                          ...
                          style={`background: linear-gradient(135deg, ${dept()?.colorFrom ?? '#334155'}, ${dept()?.colorTo ?? '#1e293b'}); box-shadow: 0 4px 12px ${dept()?.colorFrom ?? '#334155'}40;`}
                        >
                          <SkillIcon icon={dept()?.icon ?? ''} size="sm" />
                        </div>
                      ...
                      <div class="flex items-center justify-between text-xs">
                        <span class="text-amber-600 dark:text-amber-400 font-medium">в… {worker.rating}</span>
      ...
      <div class="absolute bottom-24 left-4 right-4 z-10">
        <div class="glass rounded-3xl p-5">
          ...
          <div class="flex gap-3">
            ...
            <button class="flex-1 py-3 bg-gradient-to-r from-sky-600 to-blue-700 rounded-2xl flex items-center justify-center gap-2 shadow-lg touch-scale">
              <Icon name="message" class="text-white" size="sm" />
              <span class="font-medium text-white">{t('tracking.message')}</span>
            </button>
          </div>
          ...
          {(order, i) => {
            ...
            return (
              ...
              >
                <div class="flex items-center justify-between mb-4">
                  <div class="flex items-center gap-3">
                    <div class="w-12 h-12 rounded-2xl bg-gradient-to-br from-sky-600 to-blue-700 flex items-center justify-center">
                ...
                <div class="flex items-center justify-between pt-4 border-t border-gray-100">
                  ...
                  <span class="text-xl font-bold text-sky-600">{order.price.toLocaleString()} ₸</span>
                </div>
              </div>
            );
          }}
...
function ThemePage(props: { onBack: () => void }) {
  ...
  return (
    <div class="min-h-screen animate-fade-in">
              ...
              >
                <div class="flex items-center gap-4">
                  <div class={`w-14 h-14 rounded-2xl flex items-center justify-center ${
                    ...
                    th.id === 'dark' ? 'bg-gradient-to-br from-sky-600 to-blue-700' :
                  ...
                  <Show when={isSelected()}>
                    <div class="w-8 h-8 rounded-full bg-gradient-to-br from-sky-600 to-blue-700 flex items-center justify-center">
      ...
      <div class="p-4">
        ...
        <div class="glass rounded-3xl p-5">
          ...
          <div class="flex gap-2">
            ...
            <button class="flex-1 py-2 bg-gradient-to-r from-sky-600 to-blue-700 rounded-xl text-white text-sm font-medium">
              {t('theme.confirm')}
            </button>
          </div>
        </div>
      ...
      {/* Auto info */}
      <div class="p-4">
        <div class="glass rounded-2xl p-4 border border-sky-200/50 bg-sky-50/30">
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-xl bg-sky-100 flex items-center justify-center">
              <Icon name="zap" class="text-sky-600" size="sm" />
            </div>
            <div>
              <p class="font-medium text-gray-800">{t('theme.activeTheme')}</p>
              <p class="text-sm text-gray-500">
                <span class="inline-flex items-center gap-1"><Icon name={activeTheme() === 'dark' ? 'moon' : 'sun'} size="xs" class="inline" /> {activeTheme() === 'dark' ? t('theme.dark') : t('theme.light')}</span>
...
const contractTypes: ContractType[] = [
  { id: 'instant', name: 'Instant', nameRu: 'Срочный', icon: 'zap' as const, descriptionKey: 'contracts.descInstant', durationKey: 'contracts.durationInstant', escrow: true, fee: 15, color: 'from-amber-400 to-orange-500' },
  { id: 'short', name: 'Short', nameRu: 'Краткий', icon: 'calendar' as const, descriptionKey: 'contracts.descShort', durationKey: 'contracts.durationShort', escrow: true, fee: 12, color: 'from-blue-400 to-cyan-500' },
  { id: 'monthly', name: 'Monthly', nameRu: 'Месячный', icon: 'clock' as const, descriptionKey: 'contracts.descMonthly', durationKey: 'contracts.durationMonthly', escrow: true, fee: 10, color: 'from-sky-600 to-blue-700' },
  { id: 'subscription', name: 'Subscription', nameRu: 'Подписка', icon: 'repeat' as const, descriptionKey: 'contracts.descSubscription', durationKey: 'contracts.durationUnlimited', escrow: false, fee: 8, color: 'from-green-400 to-emerald-500' },
];

const paymentMethods = [
  { id: 'kaspi', name: 'Kaspi Pay', nameKey: null as string | null, descKey: 'payment.instantTransfer', icon: 'creditCard' as const, desc: 'Visa, Mastercard', fee: 0, popular: true },
  { id: 'card', name: null as string | null, nameKey: 'payment.card', descKey: null, icon: 'creditCard' as const, desc: 'Visa, Mastercard', fee: 2, popular: false },
  { id: 'apple', name: 'Apple Pay', nameKey: null, descKey: null, icon: 'smartphone' as const, desc: 'iPhone / iPad', fee: 0, popular: false },
  { id: 'google', name: 'Google Pay', nameKey: null, descKey: null, icon: 'smartphone' as const, desc: 'Android', fee: 0, popular: false },
  { id: 'balance', name: null, nameKey: 'payment.balance', descKey: null, icon: 'wallet' as const, desc: '15 000 ₸', fee: 0, popular: false },
];
...
function ContractsPage(props: { onNavigate: (page: string) => void }) {
  ...
  const contracts = [
    { id: '2024-001', type: 'instant', guard: 'Алексей Козлов', avatar: 'userCheck', status: 'active', start: 'Сегодня 14:00', end: '18:00', total: 20000, paid: true, progress: 65, hoursWorked: 2.5, hoursTotal: 4, phases: [
    ...
    { id: '2024-002', type: 'monthly', guard: 'Дмитрий Сидоров', avatar: 'shield', status: 'active', start: '1 февраля', end: '28 февраля', total: 180000, paid: true, progress: 22, hoursWorked: 44, hoursTotal: 200, phases: [
    ...
    { id: '2023-098', type: 'short', guard: 'Максим Иванов', avatar: 'shieldCheck', status: 'completed', start: '25 января', end: '27 января', total: 45000, paid: true, progress: 100, hoursWorked: 48, hoursTotal: 48, phases: [
      { phaseKey: 'phaseDay', phaseNum: 1, done: true },
      { phaseKey: 'phaseDay', phaseNum: 2, done: true },
      { phaseKey: 'phaseComplete', done: true },
    ]},
    { id: '2023-095', type: 'instant', guard: 'Артём Петров', avatar: 'eye', status: 'completed', start: '20 января', end: '20 января', total: 8000, paid: true, progress: 100, hoursWorked: 2, hoursTotal: 2, phases: [
  ...
  const getProgressColors = (progress: number, status: string) => {
    ...
    if (progress >= 50) return { stroke: '#0ea5e9', bg: '#e0e7ff' };
    ...
    return (
      <div class="relative" style={`width: ${props.size}px; height: ${props.size}px`}>
        ...
        <div class="absolute inset-0 flex items-center justify-center">
          <Icon name={props.icon as any} size="lg" class="text-gray-700" />
        </div>
        {/* Percentage badge */}
        <div 
    ...
    <div class="min-h-screen animate-fade-in">
      ...
      <div class="p-4">
        <div class="flex items-center justify-between mb-2">
          ...
          <button 
            class="w-10 h-10 rounded-full bg-gradient-to-br from-sky-600 to-blue-700 flex items-center justify-center shadow-lg touch-scale"
        ...
        <div class="glass rounded-3xl p-5">
          <div class="grid grid-cols-3 gap-4">
            <div class="text-center">
              <p class="text-3xl font-bold text-sky-600">2</p>
      ...
      <Show when={contracts.some(c => c.status === 'active')}>
        <div class="px-4 mb-4">
          <button
            class="w-full bg-gradient-to-r from-blue-600 to-sky-600 rounded-2xl p-4 flex items-center gap-3 shadow-lg shadow-blue-600/20 touch-scale"
                ...
                >
                  ...
                  >
                    ...
                    <Show when={isExpanded()}>
                      <div class="mt-4 pt-4 border-t border-gray-100 animate-fade-in">
                        ...
                        <div class="grid grid-cols-3 gap-3 mb-4">
                          ...
                          <div class="bg-gray-50 rounded-xl p-3 text-center">
                            <p class="text-lg font-bold text-sky-600">{contract.hoursWorked}</p>
                        ...
                        <div class="flex gap-2 flex-wrap">
                          <Show when={contract.status === 'active'}>
                            <button 
                              class="w-full py-3 mb-2 bg-gradient-to-r from-blue-500 to-sky-600 rounded-xl text-white font-medium text-sm flex items-center justify-center gap-2 touch-scale shadow-lg shadow-blue-500/30"
                          ...
                          <button class="flex-1 min-w-0 py-2.5 bg-gradient-to-r from-sky-600 to-blue-700 rounded-xl text-white font-medium text-sm flex items-center justify-center gap-2 touch-scale shadow-lg" onClick={() => props.onNavigate('chat')}>
                    ...
                    <div class="flex justify-center mt-3">
                      <div class={`w-10 h-1 rounded-full bg-gray-200 transition-all ${isExpanded() ? 'bg-sky-400' : ''}`} />
                    </div>
                  </div>
                </div>
              );
      ...
      <div class="p-4 mt-4">
        <div class="glass rounded-2xl p-4 border border-sky-200/50 bg-sky-50/30">
          <div class="flex items-start gap-3">
            <div class="w-10 h-10 rounded-xl bg-sky-100 flex items-center justify-center">
              <Icon name="lock" class="text-sky-600" size="sm" />
            </div>
            <div>
      ...
      <Switch>
        ...
        <Match when={step() === 'type'}>
          <div class="p-4 space-y-4">
                    ...
                    <div class="flex items-start gap-4">
                      ...
                      <div class="flex-1">
                        <div class="flex items-center justify-between">
                          ...
                          <Show when={isSelected()}>
                            <div class="w-6 h-6 rounded-full bg-sky-500 flex items-center justify-center">
                              <Icon name="check" class="text-white w-4 h-4" />
                            </div>
                          </Show>
                        </div>
            ...
            <button 
              class="w-full py-4 bg-gradient-to-r from-sky-600 to-blue-700 text-white rounded-2xl font-bold text-lg shadow-xl touch-scale flex items-center justify-center gap-2"
        ...
        <Match when={step() === 'details'}>
          <div class="p-4 space-y-5">
            ...
            <div class={`glass rounded-2xl p-4 border-2 border-opacity-50 ${
              ...
              contractType().id === 'monthly' ? 'border-sky-400' :
              'border-green-400'
            }`}>
              <div class="flex items-center gap-3">
            ...
            {/* Price Summary */}
            <div class="glass rounded-3xl p-5 bg-gradient-to-br from-sky-50 to-blue-50">
              ...
              <div class="space-y-2">
                ...
                <div class="border-t border-gray-200 pt-2 mt-2 flex justify-between">
                  ...
                  <span class="text-2xl font-bold text-sky-600">{total().toLocaleString()} ₸</span>
                </div>
              </div>
            </div>
          ...
          <div class="p-4 space-y-5">
            ...
            <div class="glass rounded-3xl overflow-hidden">
              <div class={`p-5 bg-gradient-to-br ${contractType().color} text-white`}>
                <div class="flex items-center gap-3 mb-3">
                  <Icon name={contractType().icon} size="xl" class="text-white" />
                  <div>
              ...
              <div class="p-5 space-y-4">
                ...
                <div class="border-t border-gray-100 pt-4">
                  <div class="flex items-center gap-3">
                    <Icon name={selectedPayment().icon} size="lg" class="text-gray-600" />
                    <div class="flex-1">
            ...
            <div class="glass rounded-2xl p-4">
              <div class="space-y-2">
                ...
                <div class="border-t border-gray-200 pt-2 mt-2 flex justify-between">
                  ...
                  <span class="text-xl font-bold text-sky-600">{total().toLocaleString()} ₸</span>
                </div>
              </div>
            </div>
            ...
            <Show when={escrowAmount() > 0}>
              <div class="glass rounded-2xl p-4 border border-sky-200 bg-sky-50/30">
                <div class="flex items-center gap-3">
                  <Icon name="lock" class="text-sky-600" />
                  <div>
                    <p class="font-medium text-sky-800">{t('contracts.escrow')}: {escrowAmount().toLocaleString()} ₸</p>
                    <p class="text-xs text-sky-600">{t('contracts.secureEscrowDesc')}</p>
                  </div>
                </div>
              </div>
            </Show>
              ...
              <div class="space-y-2">
                ...
                >
                  ...
                  <span
                    role="button"
                    tabindex="0"
                    class="text-sky-500 text-xs font-medium underline shrink-0 cursor-pointer"
          ...
          <div class="p-4 flex flex-col items-center justify-center min-h-[70vh]">
            ...
            <div class="w-full glass rounded-3xl p-5 mb-6">
              ...
              <div class="border-t border-gray-100 pt-4 flex justify-between items-center">
                ...
                <Show when={contractType().escrow}>
                  <div class="flex items-center gap-2 px-3 py-1.5 bg-sky-50 rounded-full">
                    <Icon name="lock" class="text-sky-600 w-4 h-4" />
                    <span class="text-sm font-medium text-sky-700">{t('newContract.inEscrowLabel')}</span>
                  </div>
                </Show>
                <Show when={!contractType().escrow}>
                  <div class="flex items-center gap-2 px-3 py-1.5 bg-green-50 rounded-full">
...
function DocumentVaultPage(props: { onBack: () => void }) {
  ...
  const categories = () => [
    { id: 'all', label: t('docs.catAll'), icon: 'folder', count: 24 },
    { id: 'payslip', label: t('docs.catPayslips'), icon: 'coins', count: 8 },
    { id: 'contract', label: t('docs.catContracts'), icon: 'fileText', count: 5 },
    { id: 'receipt', label: t('docs.catReceipts'), icon: 'clipboard', count: 3 },
    { id: 'certificate', label: t('docs.catCertificates'), icon: 'trophy', count: 4 },
    { id: 'id', label: t('docs.catIds'), icon: 'fingerprint', count: 1 },
    { id: 'diploma', label: t('docs.catDiplomas'), icon: 'graduationCap', count: 1 },
    { id: 'license', label: t('docs.catLicenses'), icon: 'award', count: 2 },
    { id: 'insurance', label: t('docs.catInsurance'), icon: 'shieldCheck', count: 1 },
    { id: 'tax', label: t('docs.catTax'), icon: 'gauge', count: 1 },
  ];
  ...
  const getTypeIcon = (type: Document['type']) => {
    const map: Record<Document['type'], string> = { payslip: 'coins', contract: 'fileText', receipt: 'clipboard', certificate: 'trophy', id: 'fingerprint', diploma: 'graduationCap', license: 'award', insurance: 'shieldCheck', tax: 'gauge' };
    return map[type] || 'folder';
  };

  const getTypeBg = (type: Document['type']) => {
    if (dark()) return 'from-sky-500/30 to-blue-600/30';
      ...
      <div class="p-4 pb-2">
        <div class="flex items-center gap-3 mb-1">
          ...
          <button class="w-10 h-10 rounded-full glass flex items-center justify-center touch-scale" onClick={() => { setShowUpload(true); setUploadSuccess(false); }} aria-label="Upload">
            <Icon name="uploadCloud" class="text-sky-400" size="sm" />
          </button>
        ...
        <div class="mt-2 px-1">
          ...
          <div class="h-1.5 rounded-full bg-white/20 overflow-hidden">
            <div class="h-full rounded-full bg-gradient-to-r from-sky-400 to-cyan-400 transition-all duration-500" style={{ width: '48%' }} />
          </div>
        </div>
      </div>
      ...
      <Show when={showSearch()}>
        <div class="px-4 pb-2 animate-slide-up">
          <input
            type="text"
            placeholder={t('search.placeholder')}
            class="w-full rounded-xl glass px-4 py-2.5 text-white placeholder-white/50 border border-white/20 focus:outline-none focus:ring-2 focus:ring-sky-400/50"
            value={searchQuery()}
              ...
              <div class="flex items-start gap-3">
                ...
                <div class="flex flex-col gap-1.5 flex-shrink-0">
                  ...
                  <button class="w-9 h-9 rounded-lg glass flex items-center justify-center touch-scale" title={t('docs.download')}>
                    <Icon name="download" class="text-sky-300" size="sm" />
                  </button>
              ...
              {(entry, idx) => (
                <div class="flex gap-3">
                  ...
                  <div class="flex-1 min-w-0 pb-4">
                    ...
                    <p class="text-xs text-white/50">{entry.time} В· {entry.device}</p>
                  </div>
                </div>
              )}
            </For>
        ...
        {(doc) => (
          ...
          >
            ...
            <div class="flex-1 overflow-auto p-4" onClick={e => e.stopPropagation()}>
              <p class="text-white/60 text-sm">{doc().date}{doc().sender ? ` В· ${doc().sender}` : ''}</p>
            ...
            <Switch>
              <Match when={uploadSuccess()}>
                <div class="text-center py-4">
                  ...
                  <button class="mt-4 px-6 py-2 rounded-xl bg-sky-500 text-white font-medium touch-scale" onClick={() => { setShowUpload(false); setUploadSuccess(false); }}>{t('nav.home')}</button>
                </div>
              </Match>
              <Match when={uploading()}>
                <div class="py-4">
                  ...
                  <div class="h-2 rounded-full bg-white/20 overflow-hidden">
                    <div class="h-full rounded-full bg-sky-500 transition-all duration-300" style={{ width: `${uploadProgress()}%` }} />
                  </div>
                </div>
              </Match>
              <Match when={!uploading() && !uploadSuccess()}>
                ...
                <div class="grid grid-cols-1 gap-2">
                  <button class="flex items-center gap-3 p-3 rounded-xl glass touch-scale text-left" onClick={startUpload}>
                    <Icon name="camera" class="text-sky-400" size="sm" />
                    <span class="text-white">{t('docs.takePhoto')}</span>
                  </button>
                  <button class="flex items-center gap-3 p-3 rounded-xl glass touch-scale text-left" onClick={startUpload}>
                    <Icon name="image" class="text-sky-400" size="sm" />
                    <span class="text-white">{t('docs.chooseGallery')}</span>
                  </button>
                  <button class="flex items-center gap-3 p-3 rounded-xl glass touch-scale text-left" onClick={startUpload}>
                    <Icon name="fileText" class="text-sky-400" size="sm" />
            ...
            <div class="w-full max-w-lg rounded-t-3xl glass border-t border-white/20 p-6 pb-10 animate-slide-up" onClick={e => e.stopPropagation()}>
              ...
              <div class="space-y-3">
                <button class="w-full flex items-center gap-3 p-3 rounded-xl glass touch-scale text-left">
                  <Icon name="send" class="text-sky-400" size="sm" />
                ...
                <div class="rounded-xl bg-white/10 border border-white/20 p-3 flex items-center justify-between gap-2">
                  ...
                  <button class="flex-shrink-0 py-1.5 px-3 rounded-lg bg-sky-500 text-white text-sm font-medium touch-scale" onClick={copyShareLink}>
        ...
        <div class="flex gap-2">
          <button
            class={`flex-1 py-3 rounded-xl font-medium text-sm transition-all ${
              activeTab() === 'status' ? 'bg-sky-600 text-white' : 'glass text-gray-700'
            }`}
          ...
          <button
            class={`flex-1 py-3 rounded-xl font-medium text-sm transition-all ${
              activeTab() === 'upload' ? 'bg-sky-600 text-white' : 'glass text-gray-700'
            }`}
          ...
          <button
            class={`flex-1 py-3 rounded-xl font-medium text-sm transition-all ${
              activeTab() === 'photo' ? 'bg-sky-600 text-white' : 'glass text-gray-700'
            }`}
      ...
      <Switch>
                  ...
                  >
                    <div class="flex items-center gap-4">
                      ...
                      <Show when={item.status === 'not_submitted'}>
                        <button class="px-4 py-2 bg-sky-600 text-white rounded-xl text-sm font-medium touch-scale">
                          Загрузить
                        </button>
                      </Show>
                    </div>
        ...
        <Match when={activeTab() === 'upload'}>
          <div class="px-4 space-y-4">
            <div class="glass rounded-3xl p-5 border-2 border-dashed border-sky-300">
              <div class="text-center">
                <div class="w-16 h-16 rounded-full bg-sky-100 flex items-center justify-center mx-auto mb-4">
                  <Icon name="uploadCloud" class="text-sky-600" size="lg" />
                </div>
                ...
                <button class="px-6 py-3 bg-gradient-to-r from-sky-600 to-blue-700 text-white rounded-xl font-medium touch-scale">
                  Выбрать файл
                </button>
              </div>
            </div>
        ...
        <Match when={activeTab() === 'photo'}>
          <div class="px-4 space-y-4">
            ...
            <div class="glass rounded-3xl overflow-hidden">
              ...
              <div class="p-4 bg-gradient-to-r from-sky-600 to-blue-700">
            ...
            <div class="glass rounded-2xl p-4">
              ...
              <div class="space-y-3">
                ...
                <div class="flex items-start gap-3">
                  <div class="w-7 h-7 rounded-full bg-sky-100 flex items-center justify-center flex-shrink-0 text-sm font-bold text-sky-600">4</div>
            ...
            {/* Take photo button */}
            <button class="w-full py-4 bg-gradient-to-r from-sky-600 to-blue-700 text-white rounded-2xl font-bold text-lg shadow-xl touch-scale flex items-center justify-center gap-2">
...
function AcademyGamePage(props: { onBack: () => void }) {
  ...
  const professionalModules = [
    // ═══════ 1. PLUMBING ═══════
    {
      ...
      dept: 'plumbing',
      levels: [
        { title: 'Главный кран', question: 'Первое действие при аварии водоснабжения?', image: 'wrench', options: ['Вызвать мастера', 'Перекрыть главный кран', 'Подставить ведро', 'Позвонить соседям'], correct: 1, explanation: 'Всегда перекрывайте главный кран! Это минимизирует ущерб.', titleEn: 'Main Valve', questionEn: 'First action during a water emergency?', optionsEn: ['Call a plumber', 'Shut off the main valve', 'Place a bucket', 'Call neighbors'], explanationEn: 'Always shut off the main valve! This minimizes damage.', d: 1 },
        { title: 'Давление воды', question: 'Нормальное давление воды в квартире?', image: 'droplet', options: ['0.5 бар', '1.5-4 бар', '8-10 бар', '15 бар'], correct: 1, explanation: 'Стандарт: 1.5-4 бар. Выше 6 бар нужен редуктор.', titleEn: 'Water Pressure', questionEn: 'Normal water pressure in an apartment?', optionsEn: ['0.5 bar', '1.5-4 bar', '8-10 bar', '15 bar'], explanationEn: 'Standard: 1.5-4 bar. Above 6 bar requires a pressure reducer.', d: 1 },
        ...
        { title: 'Тефлоновая лента', question: 'Как наматывать ФУМ-ленту на резьбу?', image: 'wrench', options: ['Против резьбы', 'По ходу резьбы', 'Неважно', 'Крест-накрест'], correct: 1, explanation: 'По ходу резьбы (по часовой стрелке) — чтобы не разматывалась.', titleEn: 'PTFE Tape', questionEn: 'How to wrap PTFE tape on threads?', optionsEn: ['Against the thread', 'With the thread direction', 'Doesn\'t matter', 'Criss-cross'], explanationEn: 'With the thread direction (clockwise) — so it doesn\'t unravel.', d: 1 },
        { title: 'Температура воды', question: 'Безопасная температура горячей воды?', image: 'thermometer', options: ['70В°C', '60В°C для хранения, 49В°C на выходе', '30В°C', '90В°C'], correct: 1, explanation: '60В°C убивает легионеллу, но на выходе крана ≤49В°C чтобы не обжечь.', titleEn: 'Water Temperature', questionEn: 'Safe hot water temperature?', optionsEn: ['70В°C', '60В°C for storage, 49В°C at outlet', '30В°C', '90В°C'], explanationEn: '60В°C kills Legionella, but outlet must be ≤49В°C to prevent scalding.', d: 2 },
        { title: 'Медные трубы', question: 'Почему нельзя соединять медь и сталь напрямую?', image: 'link', options: ['Разный диаметр', 'Электрохимическая коррозия', 'Слишком дорого', 'Не держит давление'], correct: 1, explanation: 'Гальваническая коррозия разрушает стык. Нужен диэлектрик.', d: 2 },
        { title: 'Обратный клапан', question: 'Где обязателен обратный клапан?', image: 'repeat', options: ['На холодной воде', 'На вводе водонагревателя', 'На сливе', 'Нигде'], correct: 1, explanation: 'Обратный клапан на бойлере предотвращает обратный поток горячей воды.', d: 2 },
        { title: 'Засор', question: 'Чем НЕЛЬЗЯ прочищать пластиковые трубы?', image: 'showerHead', options: ['Вантуз', 'Кислотные средства', 'Трос', 'Горячая вода'], correct: 1, explanation: 'Кислота может растворить пластик! Используйте щелочные средства.', d: 2 },
        { title: 'Уклон канализации', question: 'Минимальный уклон канализационной трубы 110мм?', image: 'target', options: ['0.5 см/м', '2 см/м', '5 см/м', '10 см/м'], correct: 1, explanation: 'Стандарт СНиП: 2 см на метр для трубы 110мм.', d: 2 },
        { title: 'Гидроудар', question: 'Что вызывает гидроудар?', image: 'alertTriangle', options: ['Холодная вода', 'Резкое закрытие крана', 'Фильтр', 'Низкое давление'], correct: 1, explanation: 'Быстрое закрытие создаёт ударную волну. Решение — компенсатор.', d: 3 },
        { title: 'Легионелла', question: 'При какой температуре размножается легионелла?', image: 'alertTriangle', options: ['0-10В°C', '20-45В°C', '60-80В°C', '90В°C+'], correct: 1, explanation: 'Опасная зона 20-45В°C. Бойлер держите на 60В°C минимум.', d: 3 },
        { title: 'PPR трубы', question: 'Температура пайки полипропилена?', image: 'fire', options: ['100В°C', '260В°C', '400В°C', '500В°C'], correct: 1, explanation: '260В°C — стандарт для пайки PPR труб. Перегрев сужает проход.', d: 3 },
      ]
    },
    // ═══════ 2. ELECTRICAL ═══════
    {
      ...
      dept: 'electrical',
      levels: [
        { title: 'Смертельный ток', question: 'Какой ток опасен для жизни?', image: 'zap', options: ['10 А', '0.1 А (100 мА)', '5 А', '50 А'], correct: 1, explanation: '100 мА через сердце = фибрилляция. Даже 30 мА может убить.', titleEn: 'Lethal Current', questionEn: 'What current is dangerous to life?', optionsEn: ['10 A', '0.1 A (100 mA)', '5 A', '50 A'], explanationEn: '100 mA through the heart = fibrillation. Even 30 mA can be lethal.', d: 1 },
        { title: 'УЗО', question: 'Что такое УЗО/RCD и зачем?', image: 'plug', options: ['Счётчик', 'Защита от утечки тока', 'Усилитель', 'Выключатель'], correct: 1, explanation: 'RCD отключает при утечке 30 мА за 0.03 сек — спасает жизнь.', titleEn: 'RCD/GFCI', questionEn: 'What is an RCD and what is it for?', optionsEn: ['Meter', 'Protection from current leakage', 'Amplifier', 'Switch'], explanationEn: 'RCD trips on 30 mA leakage in 0.03 sec — saves lives.', d: 1 },
        { title: 'Заземление', question: 'Цвет провода заземления по МЭК?', image: 'link', options: ['Красный', 'Ƹлто-зелёный', 'Синий', 'Белый'], correct: 1, explanation: 'Ƹлто-зелёный = земля (PE). Синий = нейтраль. Коричневый/чёрный = фаза.', titleEn: 'Grounding', questionEn: 'IEC ground wire color?', optionsEn: ['Red', 'Yellow-green', 'Blue', 'White'], explanationEn: 'Yellow-green = ground (PE). Blue = neutral. Brown/black = live.', d: 1 },
        { title: 'Автомат', question: 'Когда срабатывает автомат на 16А?', image: 'wrench', options: ['Всегда', 'При перегрузке или КЗ', 'При низком напряжении', 'Никогда'], correct: 1, explanation: 'При перегрузке (тепловой расцеп.) и коротком замыкании (электромагн.).', titleEn: 'Circuit Breaker', questionEn: 'When does a 16A breaker trip?', optionsEn: ['Always', 'On overload or short circuit', 'On low voltage', 'Never'], explanationEn: 'On overload (thermal) and short circuit (electromagnetic).', d: 1 },
        { title: 'Сечение провода', question: 'Сечение кабеля для розеток 16А?', image: 'target', options: ['1.0 мм²', '1.5 мм²', '2.5 мм²', '4.0 мм²'], correct: 2, explanation: '2.5 мм² для розеток (16А). 1.5 мм² — для освещения (10А).', titleEn: 'Wire Gauge', questionEn: 'Cable cross-section for 16A outlets?', optionsEn: ['1.0 mmВІ', '1.5 mmВІ', '2.5 mmВІ', '4.0 mmВІ'], explanationEn: '2.5 mmВІ for outlets (16A). 1.5 mmВІ for lighting (10A).', d: 2 },
        { title: 'Дуга', question: 'Что вызывает электрическую дугу?', image: 'fire', options: ['Мокрые руки', 'Плохой контакт/зазор', 'Длинный кабель', 'Тёмное помещение'], correct: 1, explanation: 'Плохой контакт, окисление, ослабленный зажим = искра → пожар.', titleEn: 'Arc Flash', questionEn: 'What causes an electrical arc?', optionsEn: ['Wet hands', 'Poor contact/gap', 'Long cable', 'Dark room'], explanationEn: 'Poor contact, oxidation, loose terminal = spark → fire.', d: 2 },
        { title: 'Мокрые помещения', question: 'Класс защиты розетки в ванной?', image: 'showerHead', options: ['IP20', 'IP44', 'IP65', 'IP00'], correct: 1, explanation: 'IP44 минимум. Зона 0 (душ) — никаких розеток. Зона 2 — IP44+.', titleEn: 'Wet Rooms', questionEn: 'IP rating for bathroom outlet?', optionsEn: ['IP20', 'IP44', 'IP65', 'IP00'], explanationEn: 'IP44 minimum. Zone 0 (shower) — no outlets. Zone 2 — IP44+.', d: 2 },
        { title: 'LOTO', question: 'Что такое LOTO в электрике?', image: 'lock', options: ['Лотерея', 'Lockout/Tagout', 'Тип провода', 'Лампа'], correct: 1, explanation: 'Lock Out / Tag Out — блокировка и маркировка перед работой. Стандарт OSHA.', titleEn: 'LOTO', questionEn: 'What is LOTO in electrical work?', optionsEn: ['Lottery', 'Lockout/Tagout', 'Wire type', 'Lamp'], explanationEn: 'Lock Out / Tag Out — lock and tag before work. OSHA standard.', d: 2 },
        { title: 'Кондиционер линия', question: 'Нужна ли отдельная линия для кондиционера?', image: 'snowflake', options: ['Нет, любая розетка', 'Да, отдельный автомат', 'Через удлинитель', 'Через соседнюю розетку'], correct: 1, explanation: 'Кондиционер требует отдельную линию с автоматом и УЗО.', titleEn: 'AC Circuit', questionEn: 'Does an AC unit need a dedicated circuit?', optionsEn: ['No, any outlet', 'Yes, dedicated breaker', 'Via extension cord', 'From adjacent outlet'], explanationEn: 'AC requires a dedicated circuit with breaker and RCD.', d: 2 },
        { title: 'Электроожог', question: 'Первая помощь при электроожоге?', image: 'alertCircle', options: ['Мазь', 'Отключить ток, CPR, 112', 'Вода', 'Растирание'], correct: 1, explanation: 'Отключить источник! Не трогать голыми руками. CPR при остановке сердца.', titleEn: 'Electric Shock', questionEn: 'First aid for electric shock?', optionsEn: ['Ointment', 'Cut power, CPR, 112', 'Water', 'Rub'], explanationEn: 'Cut power source! Don\'t touch with bare hands. CPR if cardiac arrest.', d: 3 },
        { title: 'Фаза на выключатель', question: 'Почему фазу ведут через выключатель?', image: 'lightbulb', options: ['Для экономии', 'Чтобы при выключении лампа была обесточена', 'Неважно', 'Для яркости'], correct: 1, explanation: 'Если нейтраль через выключатель — патрон под напряжением даже выключенный!', d: 3 },
        { title: 'Селективность', question: 'Что такое селективность автоматов?', image: 'gauge', options: ['Один автомат', 'Отключается только ближайший к аварии', 'Все отключаются', 'Дизайн щита'], correct: 1, explanation: 'При КЗ срабатывает только автомат на повреждённой линии, остальные работают.', titleEn: 'Selectivity', questionEn: 'What is breaker selectivity?', optionsEn: ['One breaker', 'Only the breaker nearest the fault trips', 'All trip', 'Panel design'], explanationEn: 'On short circuit only the faulted circuit\'s breaker trips, others stay on.', d: 3 },
      ]
    },
    // ═══════ 3. LOCKS & DOORS ═══════
    {
    ...
    // ═══════ 4. TECH REPAIR ═══════
    {
      ...
      dept: 'tech',
      levels: [
        { title: 'ESD', question: 'Что такое ESD и почему опасно?', image: 'zap', options: ['Экран', 'Статический разряд — убивает микросхемы', 'Программа', 'Ошибка'], correct: 1, explanation: 'ESD: 20В убивает чип. Человек чувствует только от 3000В!', titleEn: 'ESD', questionEn: 'What is ESD and why is it dangerous?', optionsEn: ['Screen', 'Static discharge — kills microchips', 'Program', 'Error'], explanationEn: 'ESD: 20V kills a chip. Humans only feel from 3000V!', d: 1 },
        { title: 'Антистатика', question: 'Обязательное при ремонте ПК внутри?', image: 'monitor', options: ['Перчатки', 'Антистатический браслет', 'Шапка', 'Ботинки'], correct: 1, explanation: 'ESD-браслет заземляет вас и защищает компоненты.', titleEn: 'Antistatic', questionEn: 'Required when repairing PC internals?', optionsEn: ['Gloves', 'Antistatic wrist strap', 'Cap', 'Boots'], explanationEn: 'ESD wrist strap grounds you and protects components.', d: 1 },
        { title: 'Мультиметр', question: 'Чем измеряют напряжение?', image: 'gauge', options: ['Отвёрткой', 'Мультиметром', 'Термометром', 'Линейкой'], correct: 1, explanation: 'Мультиметр: напряжение, ток, сопротивление — основной инструмент.', titleEn: 'Multimeter', questionEn: 'What measures voltage?', optionsEn: ['Screwdriver', 'Multimeter', 'Thermometer', 'Ruler'], explanationEn: 'Multimeter: voltage, current, resistance — essential tool.', d: 1 },
        { title: 'Конденсатор', question: 'Чем опасен конденсатор в телевизоре/микроволновке?', image: 'alertTriangle', options: ['Запахом', 'Хранит смертельный заряд даже выключенный', 'Весом', 'Температурой'], correct: 1, explanation: 'Конденсатор в СВЧ = до 4000В! Всегда разряжайте перед работой.', titleEn: 'Capacitor', questionEn: 'Why is a capacitor in a TV/microwave dangerous?', optionsEn: ['Smell', 'Holds lethal charge even when off', 'Weight', 'Temperature'], explanationEn: 'Capacitor in microwave = up to 4000V! Always discharge before work.', d: 2 },
        { title: 'Термопаста', question: 'Как часто менять термопасту на CPU?', image: 'thermometer', options: ['Никогда', 'Каждый месяц', 'Раз в 2-3 года', 'Каждый день'], correct: 2, explanation: 'Каждые 2-3 года. Высохшая паста = перегрев = throttling.', titleEn: 'Thermal Paste', questionEn: 'How often to replace thermal paste on CPU?', optionsEn: ['Never', 'Every month', 'Every 2-3 years', 'Every day'], explanationEn: 'Every 2-3 years. Dried paste = overheating = throttling.', d: 2 },
        { title: 'Компрессор', question: 'Чем чистить пыль внутри ПК?', image: 'wind', options: ['Пылесосом', 'Сжатым воздухом', 'Мокрой тряпкой', 'Феном'], correct: 1, explanation: 'Сжатый воздух! Пылесос создаёт статику. Фен — горячий воздух.', titleEn: 'Compressed Air', questionEn: 'How to clean dust inside a PC?', optionsEn: ['Vacuum', 'Compressed air', 'Wet cloth', 'Hair dryer'], explanationEn: 'Compressed air! Vacuum creates static. Hair dryer = hot air.', d: 1 },
        ...
        { title: 'Правильная отвёртка', question: 'Почему важен правильный размер отвёртки?', image: 'wrench', options: ['Скорость', 'Не сорвать шлиц/головку', 'Красота', 'Неважно'], correct: 1, explanation: 'Неподходящая отвёртка срывает шлиц. Потом болт не выкрутить!', titleEn: 'Correct Screwdriver', questionEn: 'Why is the correct screwdriver size important?', optionsEn: ['Speed', 'Avoid stripping the head', 'Appearance', 'Doesn\'t matter'], explanationEn: 'Wrong size strips the screw head. Then the bolt cannot be removed!', d: 1 },
        { title: 'Резервная копия', question: 'Правило 3-2-1 для бэкапов?', image: 'hardDrive', options: ['3 файла', '3 копии, 2 типа носителя, 1 вне здания', 'Раз в 3 дня', '3 диска'], correct: 1, explanation: '3 копии данных, на 2 типах носителей, 1 копия в другом месте.', d: 3 },
        { title: 'SSD vs HDD', question: 'Можно ли восстановить данные с SSD после TRIM?', image: 'hardDrive', options: ['Легко', 'Практически невозможно', 'Всегда', 'С программой'], correct: 1, explanation: 'TRIM обнуляет ячейки. Восстановление после TRIM почти невозможно!', d: 3 },
        { title: 'Пайка', question: 'Температура пайки электроники (бессвинцовый)?', image: 'fire', options: ['100В°C', '250В°C', '350-370В°C', '500В°C'], correct: 2, explanation: '350-370В°C для бессвинцового припоя. Свинцовый: 300-320В°C.', d: 3 },
      ]
    },
    // ═══════ 5. HANDYMAN ═══════
    {
    ...
    // ═══════ 6. CLEANING ═══════
    {
      ...
      dept: 'cleaning',
      levels: [
        { title: 'Направление уборки', question: 'Правильный порядок уборки помещения?', image: 'sparkles', options: ['От пола к потолку', 'Сверху вниз, от дальнего угла к выходу', 'Случайно', 'Только пол'], correct: 1, explanation: 'Сверху вниз — пыль падает. От дальнего угла — не наступать на чистое.', titleEn: 'Cleaning direction', questionEn: 'Correct order for cleaning a room?', optionsEn: ['From floor to ceiling', 'Top to bottom, from far corner to exit', 'Random', 'Floor only'], explanationEn: 'Top to bottom — dust falls down. From far corner — avoid stepping on clean areas.', d: 1 },
        { title: 'Микрофибра', question: 'Почему микрофибра лучше обычной тряпки?', image: 'droplet', options: ['Дешевле', 'Собирает бактерии без химии', 'Ярче', 'Тяжелее'], correct: 1, explanation: 'Микрофибра захватывает 99% бактерий. Обычная тряпка размазывает грязь.', titleEn: 'Microfiber', questionEn: 'Why is microfiber better than a regular cloth?', optionsEn: ['Cheaper', 'Captures bacteria without chemicals', 'Brighter', 'Heavier'], explanationEn: 'Microfiber captures 99% of bacteria. Regular cloth spreads dirt around.', d: 1 },
        { title: 'Хлорка + аммиак', question: 'Что нельзя смешивать с хлоркой?', image: 'alertTriangle', options: ['Воду', 'Аммиак (нашатырь)', 'Соду', 'Мыло'], correct: 1, explanation: 'Хлорка + аммиак = хлорамин, токсичный газ! Может убить.', titleEn: 'Bleach + ammonia', questionEn: 'What must never be mixed with bleach?', optionsEn: ['Water', 'Ammonia', 'Baking soda', 'Soap'], explanationEn: 'Bleach + ammonia = chloramine, toxic gas! Can be fatal.', d: 1 },
        { title: 'Время дезинфекции', question: 'Сколько дезинфектант должен оставаться на поверхности?', image: 'clock', options: ['1 секунда', '5-10 минут (по инструкции)', 'Мгновенно', '1 час'], correct: 1, explanation: 'Contact time — время контакта. Обычно 5-10 минут для убийства бактерий.', titleEn: 'Disinfection time', questionEn: 'How long must disinfectant stay on the surface?', optionsEn: ['1 second', '5-10 minutes (per instructions)', 'Instantly', '1 hour'], explanationEn: 'Contact time is critical. Usually 5-10 minutes to kill bacteria.', d: 2 },
        { title: 'Цветовая кодировка', question: 'Красная тряпка в международной системе?', image: 'circleDot', options: ['Кухня', 'Санузлы', 'Офис', 'Стёкла'], correct: 1, explanation: 'Красный = санузлы. Синий = общие. Зелёный = кухня. Ƹлтый = изоляция.', titleEn: 'Color coding', questionEn: 'What does a red cloth mean in the international system?', optionsEn: ['Kitchen', 'Restrooms', 'Office', 'Glass'], explanationEn: 'Red = restrooms. Blue = general. Green = kitchen. Yellow = isolation.', d: 2 },
        { title: 'Каменная столешница', question: 'Чем НЕЛЬЗЯ мыть мрамор?', image: 'droplet', options: ['Мыльной водой', 'Кислотными средствами (уксус)', 'Тёплой водой', 'Специальным средством'], correct: 1, explanation: 'Кислота растворяет мрамор! Только pH-нейтральные средства.', titleEn: 'Stone countertop', questionEn: 'What must NOT be used to clean marble?', optionsEn: ['Soapy water', 'Acidic cleaners (vinegar)', 'Warm water', 'Special cleaner'], explanationEn: 'Acid dissolves marble! Use only pH-neutral products.', d: 2 },
        { title: 'HEPA фильтр', question: 'Что задерживает HEPA-фильтр?', image: 'wind', options: ['Только пыль', '99.97% частиц ≥0.3 мкм', '50% бактерий', 'Запахи'], correct: 1, explanation: 'HEPA: 99.97% частиц 0.3 микрон и больше. Обязателен для аллергиков.', titleEn: 'HEPA filter', questionEn: 'What does a HEPA filter capture?', optionsEn: ['Dust only', '99.97% of particles ≥0.3 Вµm', '50% of bacteria', 'Odors'], explanationEn: 'HEPA: 99.97% of particles 0.3 microns and larger. Essential for allergy sufferers.', d: 2 },
        { title: 'Ковёр', question: 'Метод горячей экстракции ковров?', image: 'sparkles', options: ['Пылесос', 'Горячая вода + химия → вакуум', 'Стирка в машинке', 'Выбивание'], correct: 1, explanation: 'Горячая экстракция: 60-70В°C раствор впрыскивается и тут же всасывается.', titleEn: 'Carpet', questionEn: 'What is hot water extraction for carpets?', optionsEn: ['Vacuum only', 'Hot water + chemicals injected then vacuumed', 'Machine wash', 'Beating'], explanationEn: 'Hot extraction: 60-70В°C solution is injected and immediately vacuumed up.', d: 2 },
        { title: 'Биологические', question: 'Как убирать биологические жидкости (кровь)?', image: 'рџ©ё', options: ['Обычной тряпкой', 'В СИЗ + дезинфекция + спецпакет', 'Водой', 'Игнорировать'], correct: 1, explanation: 'Перчатки + маска. Дезинфекция хлоросодержащим. Утилизация в биопакет.', d: 3 },
        { title: 'Плесень', question: 'Как правильно удалить плесень?', image: 'alertTriangle', options: ['Протереть', 'Антиплесневое + обработать причину влаги', 'Закрасить', 'Пылесосом'], correct: 1, explanation: 'Убить плесень + устранить источник влаги. Иначе вернётся через неделю.', d: 3 },
        { title: 'SDS клининг', question: 'Что такое SDS для моющего средства?', image: 'clipboard', options: ['Реклама', 'Паспорт безопасности (Safety Data Sheet)', 'Цена', 'Рецепт'], correct: 1, explanation: 'SDS: состав, опасности, первая помощь, хранение. Обязателен по закону.', d: 3 },
      ]
    },
    // ═══════ 7. MOVING & DELIVERY ═══════
    {
      ...
      dept: 'moving',
      levels: [
        { title: 'Подъём груза', question: 'Правильная техника подъёма тяжёлого?', image: 'box', options: ['Спиной, наклонясь', 'Ногами, спина прямая', 'Одной рукой', 'Рывком'], correct: 1, explanation: 'Ногами! Спина прямая, колени согнуты. Спина ≠ подъёмный кран.', titleEn: 'Lifting loads', questionEn: 'Correct technique for lifting heavy objects?', optionsEn: ['Back bent', 'Legs, back straight', 'One hand', 'Jerk'], explanationEn: 'Use legs! Back straight, knees bent. Your back is not a crane.', d: 1 },
        { title: 'Максимальный вес', question: 'Макс. вес для одного человека (ISO 11228)?', image: 'gauge', options: ['50 кг', '25 кг', '100 кг', '10 кг'], correct: 1, explanation: 'ISO 11228: максимум 25 кг для мужчин в идеальных условиях.', titleEn: 'Maximum weight', questionEn: 'Max weight for one person (ISO 11228)?', optionsEn: ['50 kg', '25 kg', '100 kg', '10 kg'], explanationEn: 'ISO 11228: maximum 25 kg for men under ideal conditions.', d: 1 },
        { title: 'Холодильник', question: 'Как перевозить холодильник?', image: 'box', options: ['Лёжа', 'Строго вертикально или под углом ≤45В°', 'Вверх ногами', 'На боку'], correct: 1, explanation: 'Вертикально! Если лёжа — масло вытечет из компрессора. После перевозки ждать 4ч.', titleEn: 'Refrigerator', questionEn: 'How to transport a refrigerator?', optionsEn: ['Lying down', 'Strictly upright or at angle ≤45В°', 'Upside down', 'On its side'], explanationEn: 'Upright! If horizontal — oil leaks from compressor. Wait 4 hours after moving before plugging in.', d: 1 },
        { title: 'Стекло', question: 'Как транспортировать стекло/зеркало?', image: 'рџЄџ', options: ['Горизонтально', 'Вертикально, в пузырчатой плёнке', 'В газете', 'Без упаковки'], correct: 1, explanation: 'Всегда вертикально + пузырчатая плёнка + картонные углы. Горизонтально = трещина.', titleEn: 'Glass', questionEn: 'How to transport glass/mirror?', optionsEn: ['Horizontally', 'Vertically, in bubble wrap', 'In newspaper', 'Unpackaged'], explanationEn: 'Always vertical + bubble wrap + cardboard corners. Horizontal = crack.', d: 1 },
        { title: 'Стрейч-плёнка', question: 'Зачем оборачивать мебель стрейч-плёнкой?', image: 'clipboard', options: ['Для красоты', 'Защита от царапин + фиксация дверей/ящиков', 'Для веса', 'Не нужно'], correct: 1, explanation: 'Стрейч: защита + фиксация выдвижных частей. ĸшево и эффективно.', titleEn: 'Stretch film', questionEn: 'Why wrap furniture in stretch film?', optionsEn: ['For looks', 'Scratch protection + secures doors/drawers', 'For weight', 'Not needed'], explanationEn: 'Stretch film: protection + secures moving parts. Cheap and effective.', d: 2 },
        { title: 'Развесовка', question: 'Как правильно загружать фургон?', image: 'truck', options: ['Как попало', 'Тяжёлое внизу у кабины, лёгкое сверху', 'Тяжёлое сверху', 'Лёгкое первым'], correct: 1, explanation: 'Тяжёлое: низ + ближе к кабине = устойчивость. Лёгкое и хрупкое — сверху.', titleEn: 'Load distribution', questionEn: 'How to correctly load a van?', optionsEn: ['Randomly', 'Heavy at bottom near cab, light on top', 'Heavy on top', 'Light first'], explanationEn: 'Heavy: low + near cab = stability. Light and fragile — on top.', d: 2 },
        { title: 'Ремни крепления', question: 'Чем фиксировать груз в кузове?', image: 'link', options: ['Ничем', 'Стяжные ремни (рэтчеты)', 'Верёвкой', 'Надеяться на лучшее'], correct: 1, explanation: 'Рэтчеты (стяжные ремни) с сертификатом. Верёвка ненадёжна!', titleEn: 'Cargo straps', questionEn: 'How to secure cargo in the van?', optionsEn: ['Nothing', 'Ratchet straps', 'Rope', 'Hope for the best'], explanationEn: 'Certified ratchet straps. Rope is unreliable!', d: 2 },
        { title: 'Стиральная машина', question: 'Что обязательно перед перевозкой стиралки?', image: 'plug', options: ['Ничего', 'Закрутить транспортировочные болты', 'Заполнить водой', 'Снять дверцу'], correct: 1, explanation: 'Транспортировочные болты фиксируют барабан. Без них = поломка при тряске.', titleEn: 'Washing machine', questionEn: 'What is required before transporting a washing machine?', optionsEn: ['Nothing', 'Install transport bolts', 'Fill with water', 'Remove door'], explanationEn: 'Transport bolts secure the drum. Without them = damage from vibration.', d: 2 },
        { title: 'Пианино', question: 'Минимум людей для перевозки пианино?', image: 'piano', options: ['1', '2', '4 человека + оборудование', '10'], correct: 2, explanation: 'Пианино: 200-400 кг. Минимум 4 человека + рохля/ремни. Нет места для ошибки.', titleEn: 'Piano', questionEn: 'Minimum people to move a piano?', optionsEn: ['1', '2', '4 people + equipment', '10'], explanationEn: 'Piano: 200-400 kg. Minimum 4 people + dolly/straps. No room for error.', d: 3 },
        { title: 'Лифт грузоподъёмность', question: 'Средняя грузоподъёмность пассажирского лифта?', image: 'arrowUp', options: ['100 кг', '400-630 кг', '2000 кг', '50 кг'], correct: 1, explanation: '400-630 кг типичный. НИКОГДА не перегружать! Проверяйте табличку.', titleEn: 'Elevator capacity', questionEn: 'Typical passenger elevator capacity?', optionsEn: ['100 kg', '400-630 kg', '2000 kg', '50 kg'], explanationEn: '400-630 kg typical. NEVER overload! Check the placard.', d: 2 },
        { title: 'Документы', question: 'Какие документы при коммерческой перевозке?', image: 'clipboard', options: ['Никаких', 'ТТН + опись + страховка', 'Только паспорт', 'Визитка'], correct: 1, explanation: 'Товарно-транспортная накладная + опись имущества + страхование.', titleEn: 'Documents', questionEn: 'What documents for commercial moving?', optionsEn: ['None', 'Bill of lading + inventory + insurance', 'Passport only', 'Business card'], explanationEn: 'Bill of lading + property inventory + insurance.', d: 3 },
      ]
    },
  ...
  const generalModules = [
    ...
    },
    {
      ...
      levels: [
        { title: 'Кровотечение', question: 'Первое действие при кровотечении?', image: 'рџ©ё', options: ['Жгут', 'Прямое давление', 'Поднять конечность', 'Промыть'], correct: 1, explanation: 'Прямое давление — первый метод.', titleEn: 'Bleeding', questionEn: 'First action for bleeding?', optionsEn: ['Tourniquet', 'Direct pressure', 'Elevate limb', 'Rinse'], explanationEn: 'Direct pressure — first-line method.', d: 1 },
        { title: 'Ожоги', question: 'Как охлаждать ожог?', image: 'fire', options: ['Льдом', 'Прохладной водой 10-20 мин', 'Маслом', 'Зубной пастой'], correct: 1, explanation: 'Прохладная вода 10-20 минут.', titleEn: 'Burns', questionEn: 'How to cool a burn?', optionsEn: ['Ice', 'Cool water 10-20 min', 'Oil', 'Toothpaste'], explanationEn: 'Cool running water 10-20 minutes.', d: 1 },
        { title: 'Recovery', question: 'Когда применять боковое положение?', image: 'bed', options: ['При переломе', 'Без сознания, но дышит', 'При сердечном', 'При кровотечении'], correct: 1, explanation: 'Recovery position — для без сознания, но дышащих.', titleEn: 'Recovery Position', questionEn: 'When to use recovery position?', optionsEn: ['For fracture', 'Unconscious but breathing', 'Cardiac arrest', 'Bleeding'], explanationEn: 'Recovery position — for unconscious but breathing victims.', d: 1 },
        { title: 'Перелом', question: 'Главное правило при переломе?', image: 'activity', options: ['Вправить', 'Иммобилизация', 'Массаж', 'Нагрузить'], correct: 1, explanation: 'Иммобилизация — обездвиживание.', titleEn: 'Fracture', questionEn: 'Main rule for a fracture?', optionsEn: ['Reduce it', 'Immobilization', 'Massage', 'Load it'], explanationEn: 'Immobilization — prevent movement of the injured part.', d: 1 },
        { title: 'CPR Компрессии', question: 'Глубина компрессий при СЛР взрослого?', image: 'heart', options: ['2-3 см', '5-6 см', '8-10 см', '1-2 см'], correct: 1, explanation: 'AHA/ERC 2020: 5-6 см, 100-120 в минуту.', titleEn: 'CPR Compressions', questionEn: 'Depth of compressions for adult CPR?', optionsEn: ['2-3 cm', '5-6 cm', '8-10 cm', '1-2 cm'], explanationEn: 'AHA/ERC 2020: 5-6 cm, 100-120 per minute.', d: 2 },
        { title: 'Соотношение CPR', question: 'Соотношение компрессий к вдохам?', image: 'рџ«Ѓ', options: ['15:2', '30:2', '15:1', '10:2'], correct: 1, explanation: '30:2 — международный стандарт.', titleEn: 'CPR Ratio', questionEn: 'Compression-to-breath ratio?', optionsEn: ['15:2', '30:2', '15:1', '10:2'], explanationEn: '30:2 — international standard (AHA/ERC).', d: 2 },
        { title: 'Геймлих', question: 'Куда направлены толчки Геймлиха?', image: 'alertCircle', options: ['В спину', 'В грудь', 'Внутрь и вверх', 'Вниз'], correct: 2, explanation: 'Внутрь и вверх, под диафрагму.', titleEn: 'Heimlich', questionEn: 'In what direction are Heimlich thrusts?', optionsEn: ['To the back', 'To the chest', 'Inward and upward', 'Downward'], explanationEn: 'Inward and upward, below the diaphragm.', d: 2 },
        { title: 'FAST тест', question: 'Что проверяет "F" в тесте FAST?', image: 'cpu', options: ['Fingers', 'Face', 'Feet', 'Focus'], correct: 1, explanation: 'FAST: Face, Arms, Speech, Time.', titleEn: 'FAST Test', questionEn: 'What does "F" check in FAST stroke test?', optionsEn: ['Fingers', 'Face', 'Feet', 'Focus'], explanationEn: 'FAST: Face, Arms, Speech, Time.', d: 2 },
        { title: 'ABC', question: 'Что означает ABC?', image: 'fileText', options: ['Always Be Careful', 'Airway-Breathing-Circulation', 'Alert-Blood-Check', 'Assess-Bandage-Call'], correct: 1, explanation: 'Airway, Breathing, Circulation.', titleEn: 'ABC', questionEn: 'What does ABC stand for?', optionsEn: ['Always Be Careful', 'Airway-Breathing-Circulation', 'Alert-Blood-Check', 'Assess-Bandage-Call'], explanationEn: 'Airway, Breathing, Circulation.', d: 2 },
        { title: 'Шок', question: 'Позиция при шоке?', image: 'alertCircle', options: ['Сидя', 'Ноги выше головы', 'На животе', 'Стоя'], correct: 1, explanation: 'Ноги приподняты.', titleEn: 'Shock', questionEn: 'Position for shock?', optionsEn: ['Sitting', 'Legs elevated above heart', 'On stomach', 'Standing'], explanationEn: 'Legs elevated (unless contraindicated).', d: 2 },
        { title: 'AED', question: 'Можно ли AED на мокром человеке?', image: 'zap', options: ['Да', 'Нет', 'Сначала вытереть', 'Только в воде'], correct: 2, explanation: 'Вытереть грудь насухо!', titleEn: 'AED', questionEn: 'Can you use AED on a wet person?', optionsEn: ['Yes', 'No', 'Dry chest first', 'Only in water'], explanationEn: 'Dry the chest first!', d: 3 },
        { title: 'Анафилаксия', question: 'Куда вводят адреналин?', image: 'zap', options: ['В вену', 'В бедро', 'В ягодицу', 'Под язык'], correct: 1, explanation: 'EpiPen — в бедро.', titleEn: 'Anaphylaxis', questionEn: 'Where to inject epinephrine?', optionsEn: ['Intravenously', 'Into thigh', 'Into buttock', 'Under tongue'], explanationEn: 'EpiPen — into the thigh.', d: 3 },
        { title: 'CPR детям', question: 'Глубина компрессий для ребёнка?', image: 'heart', options: ['2 см', '4-5 см', '6-7 см', '1 см'], correct: 1, explanation: '4-5 см, 1/3 грудной клетки.', titleEn: 'CPR for Children', questionEn: 'Compression depth for child?', optionsEn: ['2 cm', '4-5 cm', '6-7 cm', '1 cm'], explanationEn: '4-5 cm, 1/3 of chest depth (AHA/ERC).', d: 3 },
        { title: 'Укус змеи', question: 'Что НЕЛЬЗЯ при укусе змеи?', image: 'alertTriangle', options: ['Обездвижить', 'Высасывать яд', 'Вызвать скорую', 'Снять украшения'], correct: 1, explanation: 'Никогда не высасывайте яд!', titleEn: 'Snake Bite', questionEn: 'What must you NOT do for snake bite?', optionsEn: ['Immobilize', 'Suck out venom', 'Call emergency', 'Remove jewelry'], explanationEn: 'Never suck out venom!', d: 3 },
        { title: 'Тепловой удар', question: 'Признак теплового удара?', image: 'sun', options: ['Потливость', 'Горячая СУХАЯ кожа', 'Озноб', 'Голод'], correct: 1, explanation: 'Кожа горячая и сухая!', titleEn: 'Heat Stroke', questionEn: 'Sign of heat stroke?', optionsEn: ['Sweating', 'Hot DRY skin', 'Chills', 'Hunger'], explanationEn: 'Skin is hot and dry!', d: 3 },
        { title: 'Отравление', question: 'При неизвестном отравлении?', image: 'alertTriangle', options: ['Вызвать рвоту', 'Молоко', 'НЕ рвоту, звонить 112', 'Уголь'], correct: 2, explanation: 'Не вызывать рвоту!', titleEn: 'Poisoning', questionEn: 'For unknown poisoning?', optionsEn: ['Induce vomiting', 'Give milk', 'Do NOT induce vomiting, call 112/911', 'Charcoal'], explanationEn: 'Do not induce vomiting! Call poison control.', d: 3 },
        { title: 'Нос', question: 'Как остановить носовое кровотечение?', image: 'droplet', options: ['Голову назад', 'Вперёд, зажать', 'Лечь', 'Вату глубоко'], correct: 1, explanation: 'Голова вперёд + зажать.', titleEn: 'Nosebleed', questionEn: 'How to stop a nosebleed?', optionsEn: ['Tilt head back', 'Lean forward, pinch', 'Lie down', 'Stuff cotton deep'], explanationEn: 'Lean forward + pinch nostrils.', d: 2 },
        { title: 'Гипогликемия', question: 'Что дать при низком сахаре?', image: 'heart', options: ['Инсулин', 'Сахар/сок', 'Солёную воду', 'Ничего'], correct: 1, explanation: 'Быстрые углеводы: сок, сахар.', titleEn: 'Hypoglycemia', questionEn: 'What to give for low blood sugar?', optionsEn: ['Insulin', 'Sugar/juice', 'Salt water', 'Nothing'], explanationEn: 'Fast-acting carbs: juice, sugar.', d: 2 }
      ]
    },
    {
      id: 'rescue',
      ...
      levels: [
        { title: 'Утопление', question: 'Приоритет при спасении тонущего?', image: 'waves', options: ['Прыгнуть', 'Бросить предмет', 'Звать', 'Ждать'], correct: 1, explanation: 'REACH-THROW-ROW-GO.', titleEn: 'Drowning', questionEn: 'Priority when rescuing a drowning person?', optionsEn: ['Jump in', 'Throw flotation device', 'Yell', 'Wait'], explanationEn: 'REACH-THROW-ROW-GO.', d: 1 },
        { title: 'Recovery', question: 'Позиция для без сознания дышащего?', image: 'bed', options: ['На спине', 'На животе', 'Боковое стабильное', 'Сидя'], correct: 2, explanation: 'Recovery position.', titleEn: 'Recovery', questionEn: 'Position for unconscious breathing victim?', optionsEn: ['On back', 'On stomach', 'Lateral recovery', 'Sitting'], explanationEn: 'Recovery position.', d: 1 },
        { title: 'Позвоночник', question: 'При травме позвоночника?', image: 'activity', options: ['Эвакуировать', 'НЕ двигать', 'Посадить', 'Массаж'], correct: 1, explanation: 'Не двигать!', titleEn: 'Spine Injury', questionEn: 'For suspected spine injury?', optionsEn: ['Evacuate', 'Do NOT move', 'Sit them up', 'Massage'], explanationEn: 'Do not move!', d: 1 },
        { title: 'Firefighter', question: 'Как нести одному?', image: 'footprints', options: ['На спине', 'Перед собой', 'Пожарный захват', 'За ноги'], correct: 2, explanation: 'Firefighter\'s carry.', titleEn: 'Firefighter Carry', questionEn: 'How to carry alone?', optionsEn: ['On back', 'In front', 'Firefighter carry', 'By legs'], explanationEn: 'Firefighter\'s carry.', d: 2 },
        { title: 'Гипотермия', question: 'Как согревать при гипотермии?', image: 'snowflake', options: ['Горячая ванна', 'Растирание', 'Постепенно от центра', 'Алкоголь'], correct: 2, explanation: 'Медленно от центра.', titleEn: 'Hypothermia', questionEn: 'How to rewarm in hypothermia?', optionsEn: ['Hot bath', 'Rubbing', 'Gradually from core', 'Alcohol'], explanationEn: 'Slowly from core outward.', d: 2 },
        { title: 'Log roll', question: 'Log roll при?', image: 'repeat', options: ['Утоплении', 'Травме позвоночника', 'Ожогах', 'Переломе ноги'], correct: 1, explanation: 'Log roll при травме позвоночника.', titleEn: 'Log Roll', questionEn: 'When to use log roll?', optionsEn: ['Drowning', 'Spine injury', 'Burns', 'Leg fracture'], explanationEn: 'Log roll for suspected spine injury.', d: 2 },
        { title: 'Жгут', question: 'Жгут максимум на?', image: 'рџ©№', options: ['10 мин', '30 мин', '1-2 часа', 'Без ограничений'], correct: 2, explanation: '1-2 часа максимум!', titleEn: 'Tourniquet', questionEn: 'Tourniquet maximum duration?', optionsEn: ['10 min', '30 min', '1-2 hours', 'No limit'], explanationEn: '1-2 hours maximum!', d: 2 },
        { title: 'Blanket drag', question: 'Когда перенос на одеяле?', image: 'bed', options: ['Всегда', 'При травме спины', 'При пожаре', 'Никогда'], correct: 2, explanation: 'Быстрая эвакуация.', titleEn: 'Blanket Drag', questionEn: 'When to use blanket drag?', optionsEn: ['Always', 'Spine injury', 'Fire evacuation', 'Never'], explanationEn: 'Rapid evacuation from danger.', d: 2 },
        { title: 'Вторичное утопление', question: '"Вторичное утопление" это?', image: 'droplet', options: ['Утонуть дважды', 'Отёк через часы', 'Страх воды', 'Судороги'], correct: 1, explanation: 'Отёк лёгких через 1-24 часа.', titleEn: 'Secondary Drowning', questionEn: 'What is "secondary drowning"?', optionsEn: ['Drown twice', 'Pulmonary edema hours later', 'Fear of water', 'Seizures'], explanationEn: 'Pulmonary edema 1-24 hours after water aspiration.', d: 3 },
        { title: 'Confined space', question: 'Первое при спасении из замкнутого?', image: 'alertCircle', options: ['Войти', 'Проверить воздух', 'Кричать', 'Ждать'], correct: 1, explanation: 'Проверьте атмосферу!', titleEn: 'Confined Space', questionEn: 'First step when rescuing from confined space?', optionsEn: ['Enter', 'Check atmosphere', 'Yell', 'Wait'], explanationEn: 'Check atmosphere first!', d: 3 },
        { title: 'Верёвка', question: 'Минимальная прочность верёвки?', image: 'link', options: ['500 кг', '1500 кг', '3000 кг', '100 кг'], correct: 2, explanation: '15 kN (~1500 кг).', titleEn: 'Rope', questionEn: 'Minimum rope strength for rescue?', optionsEn: ['500 kg', '1500 kg', '3000 kg', '100 kg'], explanationEn: '15 kN (~1500 kg) minimum.', d: 3 },
        { title: 'Электро', question: 'Как спасать от тока?', image: 'zap', options: ['Схватить', 'Отключить питание', 'Водой', 'Толкнуть'], correct: 1, explanation: 'СНАЧАЛА отключить!', titleEn: 'Electrocution', questionEn: 'How to rescue from electric shock?', optionsEn: ['Grab them', 'Cut power first', 'Water', 'Push them'], explanationEn: 'Cut power FIRST!', d: 3 },
        { title: 'Triage', question: 'Чёрный в START triage?', image: 'layers', options: ['Лёгкие', 'Срочные', 'Отложенные', 'Погибшие'], correct: 3, explanation: 'Чёрный — погибшие.', titleEn: 'Triage', questionEn: 'What does black mean in START triage?', optionsEn: ['Minor', 'Immediate', 'Delayed', 'Deceased'], explanationEn: 'Black — deceased or expectant.', d: 3 },
        { title: 'Лёд', question: 'Как спасать из-подо льда?', image: 'box', options: ['Идти', 'Ползти', 'Бежать', 'Прыгать'], correct: 1, explanation: 'Ползком, распределяя вес.', titleEn: 'Ice Rescue', questionEn: 'How to approach someone under ice?', optionsEn: ['Walk', 'Crawl', 'Run', 'Jump'], explanationEn: 'Crawl to distribute weight.', d: 3 },
        { title: 'Лавина', question: 'При накрытии лавиной?', image: 'mountain', options: ['Бежать', 'Плавать, закрыть лицо', 'Кричать', 'Лечь'], correct: 1, explanation: 'Плавательные движения + карман у лица.', titleEn: 'Avalanche', questionEn: 'When caught in avalanche?', optionsEn: ['Run', 'Swim, create air pocket at face', 'Scream', 'Lie still'], explanationEn: 'Swimming motions + create air pocket by face.', d: 3 }
      ]
    },
    {
      id: 'hazmat',
      ...
      levels: [
        { title: 'Газ', question: 'При утечке газа НЕЛЬЗЯ?', image: 'wind', options: ['Открыть окна', 'Включать свет', 'Покинуть', 'Перекрыть'], correct: 1, explanation: 'Искра = взрыв!', titleEn: 'Gas Leak', questionEn: 'What must you NOT do during gas leak?', optionsEn: ['Open windows', 'Turn on/off lights', 'Leave', 'Shut off gas'], explanationEn: 'Any spark = explosion!', d: 1 },
        { title: 'Химожог', question: 'При ожоге кислотой?', image: 'alertTriangle', options: ['Нейтрализовать', 'Вода 20+ мин', 'Протереть', 'Маслом'], correct: 1, explanation: 'Только вода!', titleEn: 'Chemical Burn', questionEn: 'For acid burn?', optionsEn: ['Neutralize', 'Flush with water 20+ min', 'Wipe off', 'Apply oil'], explanationEn: 'Flush with water only!', d: 1 },
        { title: 'GHS Пламя', question: 'Пламя в GHS означает?', image: 'fire', options: ['Токсичность', 'Коррозия', 'Воспламеняемость', 'Взрыв'], correct: 2, explanation: 'Воспламеняемость.', titleEn: 'GHS Flame', questionEn: 'Flame symbol in GHS means?', optionsEn: ['Toxicity', 'Corrosion', 'Flammability', 'Explosive'], explanationEn: 'Flammability.', d: 2 },
        { title: 'CO', question: 'Признак отравления CO?', image: 'building2', options: ['Запах', 'Вишнёвая кожа', 'Судороги', 'Кашель'], correct: 1, explanation: 'CO без запаха! Вишнёвая кожа.', titleEn: 'Carbon Monoxide', questionEn: 'Sign of CO poisoning?', optionsEn: ['Smell', 'Cherry-red skin', 'Seizures', 'Cough'], explanationEn: 'CO is odorless! Cherry-red skin is a sign.', d: 2 },
        { title: 'Радиация', question: 'Три принципа защиты?', image: 'alertTriangle', options: ['Бег-укрытие-йод', 'Время-расстояние-экран', 'Вода-еда-воздух', 'Бежать'], correct: 1, explanation: 'Time-Distance-Shielding.', titleEn: 'Radiation', questionEn: 'Three principles of radiation protection?', optionsEn: ['Run-shelter-iodine', 'Time-Distance-Shielding', 'Water-food-air', 'Run'], explanationEn: 'Time-Distance-Shielding (ALARA).', d: 2 },
        { title: 'GHS Череп', question: 'Череп в GHS означает?', image: 'alertTriangle', options: ['Радиация', 'Острая токсичность', 'Коррозия', 'Окислитель'], correct: 1, explanation: 'Острая токсичность.', titleEn: 'GHS Skull', questionEn: 'Skull symbol in GHS means?', optionsEn: ['Radiation', 'Acute toxicity', 'Corrosion', 'Oxidizer'], explanationEn: 'Acute toxicity.', d: 2 },
        { title: 'Хлор', question: 'Какой запах у хлора?', image: 'droplet', options: ['Без запаха', 'Резкий', 'Сладкий', 'Бензин'], correct: 1, explanation: 'Резкий характерный запах.', titleEn: 'Chlorine', questionEn: 'What does chlorine smell like?', optionsEn: ['Odorless', 'Pungent/bleach-like', 'Sweet', 'Gasoline'], explanationEn: 'Pungent bleach-like odor.', d: 2 },
        { title: 'NFPA 704', question: 'Синий в NFPA 704?', image: 'shield', options: ['Пожар', 'Здоровье', 'Реактивность', 'Радиация'], correct: 1, explanation: 'Синий = здоровье.', titleEn: 'NFPA 704', questionEn: 'Blue in NFPA 704 diamond means?', optionsEn: ['Fire hazard', 'Health hazard', 'Reactivity', 'Special/radiation'], explanationEn: 'Blue = health hazard.', d: 3 },
        { title: 'SDS', question: 'Сколько разделов в SDS?', image: 'clipboard', options: ['8', '12', '16', '20'], correct: 2, explanation: '16 разделов по GHS.', titleEn: 'SDS', questionEn: 'How many sections in SDS?', optionsEn: ['8', '12', '16', '20'], explanationEn: '16 sections per GHS.', d: 3 },
        { title: 'Щёлочь', question: 'Что опаснее для кожи?', image: 'alertTriangle', options: ['Кислота', 'Щёлочь', 'Одинаково', 'Ничего'], correct: 1, explanation: 'Щёлочь проникает глубже!', titleEn: 'Alkali', questionEn: 'What is more dangerous to skin?', optionsEn: ['Acid', 'Alkali', 'Equal', 'Neither'], explanationEn: 'Alkalis penetrate deeper into tissue!', d: 3 },
        { title: 'Бензол', question: 'Опасность бензола?', image: 'droplet', options: ['Ожоги', 'Рак крови', 'Слепота', 'Глухота'], correct: 1, explanation: 'Бензол — канцероген.', titleEn: 'Benzene', questionEn: 'Benzene hazard?', optionsEn: ['Burns', 'Blood cancer/leukemia', 'Blindness', 'Deafness'], explanationEn: 'Benzene is a carcinogen (leukemia).', d: 3 },
        { title: 'Ртуть', question: 'Как собирать ртуть?', image: 'droplet', options: ['Пылесосом', 'Веником', 'Спецнабором', 'Тряпкой'], correct: 2, explanation: 'Только специальными средствами!', titleEn: 'Mercury', questionEn: 'How to collect mercury spill?', optionsEn: ['Vacuum', 'Broom', 'Special spill kit', 'Cloth'], explanationEn: 'Use mercury spill kit only! Never vacuum.', d: 3 },
        { title: 'Асбест', question: 'Опасность асбеста?', image: 'wrench', options: ['Ожоги', 'Рак лёгких', 'Отравление', 'Облучение'], correct: 1, explanation: 'Рак лёгких при вдыхании.', titleEn: 'Asbestos', questionEn: 'Asbestos hazard?', optionsEn: ['Burns', 'Lung cancer', 'Poisoning', 'Radiation'], explanationEn: 'Lung cancer/mesothelioma from inhalation.', d: 3 },
        { title: 'LEL', question: 'Что такое LEL?', image: 'gauge', options: ['Освещение', 'Нижний предел взрываемости', 'Шум', 'Температура'], correct: 1, explanation: 'Lower Explosive Limit.', titleEn: 'LEL', questionEn: 'What is LEL?', optionsEn: ['Lighting', 'Lower Explosive Limit', 'Noise', 'Temperature'], explanationEn: 'Lower Explosive Limit — minimum concentration to ignite.', d: 3 },
        { title: 'Дезактивация', question: 'Порядок дезактивации?', image: 'showerHead', options: ['Одежда→душ→медпомощь', 'Душ→одежда→еда', 'Медпомощь', 'Ничего'], correct: 0, explanation: 'Одежда → душ → медпомощь.', titleEn: 'Decontamination', questionEn: 'Decontamination sequence?', optionsEn: ['Clothing removal→shower→medical', 'Shower→clothing→food', 'Medical care', 'Nothing'], explanationEn: 'Remove clothing → shower → medical evaluation.', d: 3 }
      ]
    },
    {
      id: 'security',
      ...
      levels: [
        { title: 'Патрулирование', question: 'Почему маршрут непредсказуемый?', image: 'footprints', options: ['Физнагрузка', 'Чтобы не заметили', 'Экономия', 'Правила'], correct: 1, explanation: 'Мешает злоумышленникам.', titleEn: 'Patrolling', questionEn: 'Why use unpredictable patrol routes?', optionsEn: ['Exercise', 'Deters intruders', 'Saves time', 'Rules'], explanationEn: 'Deters intruders from anticipating.', d: 1 },
        { title: 'Описание', question: 'Порядок описания подозреваемого?', image: 'eye', options: ['Одежда→Лицо', 'Сверху вниз', 'Случайно', 'Снизу вверх'], correct: 1, explanation: 'Сверху вниз.', titleEn: 'Description', questionEn: 'Order for describing a suspect?', optionsEn: ['Clothing→Face', 'Top to bottom', 'Random', 'Bottom to top'], explanationEn: 'Top to bottom.', d: 1 },
        { title: 'Доклад', question: 'Что в докладе об инциденте?', image: 'clipboard', options: ['Имя', 'Who-What-When-Where-How', 'Мнение', 'Фото'], correct: 1, explanation: '5W+H.', titleEn: 'Incident Report', questionEn: 'What to include in incident report?', optionsEn: ['Name only', 'Who-What-When-Where-How', 'Opinion', 'Photos only'], explanationEn: '5W+H format.', d: 1 },
        { title: 'SALUTE', question: '"S" в SALUTE?', image: 'eye', options: ['Speed', 'Size', 'Silence', 'Signal'], correct: 1, explanation: 'Size, Activity, Location, Unit, Time, Equipment.', titleEn: 'SALUTE', questionEn: 'What does "S" stand for in SALUTE?', optionsEn: ['Speed', 'Size', 'Silence', 'Signal'], explanationEn: 'Size, Activity, Location, Unit, Time, Equipment.', d: 2 },
        { title: 'Эскалация', question: 'Порядок эскалации?', image: 'bell', options: ['Сила сразу', 'Присутствие→Слова→Действия', 'Игнорировать', 'Оружие'], correct: 1, explanation: 'Presence → Verbal → Physical.', titleEn: 'Use of Force', questionEn: 'Escalation order?', optionsEn: ['Force immediately', 'Presence→Verbal→Physical', 'Ignore', 'Weapon'], explanationEn: 'Presence → Verbal → Physical.', d: 2 },
        { title: 'Пропуск', question: 'Что проверять на пропуске?', image: 'fingerprint', options: ['Фото', 'Документ+лицо+срок', 'Срок', 'Ничего'], correct: 1, explanation: 'Фото=лицо, срок, подлинность.', titleEn: 'Badge Check', questionEn: 'What to verify on an access badge?', optionsEn: ['Photo only', 'Document+face+validity', 'Expiry only', 'Nothing'], explanationEn: 'Photo matches face, validity, authenticity.', d: 2 },
        { title: 'Камера', question: 'Слепая зона камеры?', image: 'camera', options: ['Темнота', 'Область вне обзора', 'Дождь', 'Ночь'], correct: 1, explanation: 'Blind spot.', titleEn: 'CCTV', questionEn: 'What is a camera blind spot?', optionsEn: ['Darkness', 'Area outside field of view', 'Rain', 'Night'], explanationEn: 'Blind spot — area not covered.', d: 2 },
        { title: 'De-escalation', question: 'Ключевой приём деэскалации?', image: 'users', options: ['Кричать', 'Активное слушание', 'Угрожать', 'Игнорировать'], correct: 1, explanation: 'Активное слушание.', titleEn: 'De-escalation', questionEn: 'Key de-escalation technique?', optionsEn: ['Yelling', 'Active listening', 'Threatening', 'Ignoring'], explanationEn: 'Active listening.', d: 2 },
        { title: 'CPTED', question: 'Что такое CPTED?', image: 'building2', options: ['Оружие', 'Дизайн против преступности', 'Камеры', 'Патруль'], correct: 1, explanation: 'Crime Prevention Through Environmental Design.', titleEn: 'CPTED', questionEn: 'What is CPTED?', optionsEn: ['Weapon', 'Crime Prevention Through Environmental Design', 'Cameras', 'Patrol'], explanationEn: 'Crime Prevention Through Environmental Design.', d: 3 },
        { title: 'Сила', question: 'Сколько уровней применения силы?', image: 'shield', options: ['2', '4', '5-6', '10'], correct: 2, explanation: '5-6 уровней.', titleEn: 'Force Continuum', questionEn: 'How many use-of-force levels?', optionsEn: ['2', '4', '5-6', '10'], explanationEn: '5-6 levels typically.', d: 3 },
        { title: 'Задержание', question: 'Когда можно задержать?', image: 'x', options: ['Всегда', 'Подозрение', 'При преступлении', 'Никогда'], correct: 2, explanation: 'Только при совершении преступления.', titleEn: 'Detention', questionEn: 'When can you detain someone?', optionsEn: ['Always', 'Suspicion', 'Upon commission of crime', 'Never'], explanationEn: 'Only upon commission of a crime (citizen\'s arrest).', d: 3 },
        { title: 'Access control', question: '3 фактора аутентификации?', image: 'lock', options: ['Имя-пароль-email', 'Знание-владение-биометрия', 'Ключ-карта-код', 'Логин-пароль-телефон'], correct: 1, explanation: 'KNOW, HAVE, ARE.', titleEn: 'Access Control', questionEn: 'Three factors of authentication?', optionsEn: ['Name-password-email', 'Knowledge-Possession-Inherence', 'Key-card-code', 'Login-password-phone'], explanationEn: 'KNOW, HAVE, ARE.', d: 3 },
        { title: 'Bomb threat', question: 'При угрозе взрыва?', image: 'alertTriangle', options: ['Искать', 'Эвакуация', 'Игнорировать', 'Фото'], correct: 1, explanation: 'Эвакуация!', titleEn: 'Bomb Threat', questionEn: 'Upon bomb threat?', optionsEn: ['Search', 'Evacuate', 'Ignore', 'Take photos'], explanationEn: 'Evacuate immediately!', d: 3 },
        { title: 'Chain of custody', question: 'Chain of custody?', image: 'link', options: ['Наручники', 'Цепь доказательств', 'Охранная', 'Командная'], correct: 1, explanation: 'Документирование улик.', titleEn: 'Chain of Custody', questionEn: 'What is chain of custody?', optionsEn: ['Handcuffs', 'Documented evidence trail', 'Security chain', 'Command chain'], explanationEn: 'Documented custody of evidence.', d: 3 },
        { title: 'Duress code', question: 'Duress code?', image: 'alertCircle', options: ['Код двери', 'Секретный сигнал опасности', 'Wi-Fi', 'Номер смены'], correct: 1, explanation: 'Тайный сигнал угрозы.', titleEn: 'Duress Code', questionEn: 'What is a duress code?', optionsEn: ['Door code', 'Secret distress signal', 'Wi-Fi password', 'Shift number'], explanationEn: 'Covert signal indicating threat or coercion.', d: 3 }
      ]
    },
    {
      id: 'emergency',
      ...
      levels: [
        { title: 'Землетрясение', question: 'При землетрясении внутри?', image: 'globe', options: ['Бежать', 'Под стол', 'У окна', 'Лифт'], correct: 1, explanation: 'DROP-COVER-HOLD.', titleEn: 'Earthquake', questionEn: 'During earthquake indoors?', optionsEn: ['Run outside', 'Drop-Cover-Hold under table', 'Stand by window', 'Use elevator'], explanationEn: 'DROP-COVER-HOLD — drop, take cover, hold on.', d: 1 },
        { title: 'Торнадо', question: 'Где укрыться от торнадо?', image: 'wind', options: ['У окна', 'Подвал/внутренняя комната', 'Крыша', 'Машина'], correct: 1, explanation: 'Подвал, нижний этаж.', titleEn: 'Tornado', questionEn: 'Where to shelter from tornado?', optionsEn: ['By window', 'Basement/inner room', 'Roof', 'Car'], explanationEn: 'Basement or lowest floor, interior room.', d: 1 },
        { title: 'Pandemic', question: 'Главная защита от инфекций?', image: 'alertTriangle', options: ['Витамины', 'Гигиена рук', 'Антибиотики', 'Бег'], correct: 1, explanation: '20+ секунд мытья рук.', titleEn: 'Pandemic', questionEn: 'Primary protection from infections?', optionsEn: ['Vitamins', 'Hand hygiene', 'Antibiotics', 'Running'], explanationEn: '20+ seconds handwashing.', d: 1 },
        { title: 'Blackout', question: 'При отключении света?', image: 'lightbulb', options: ['Паника', 'Проверить автоматы', 'Кричать', 'Бежать'], correct: 1, explanation: 'Проверьте автоматы.', titleEn: 'Blackout', questionEn: 'During power outage?', optionsEn: ['Panic', 'Check circuit breakers', 'Scream', 'Run'], explanationEn: 'Check circuit breakers.', d: 1 },
        { title: 'Цунами', question: 'Признак цунами?', image: 'waves', options: ['Ветер', 'Море отступает', 'Дождь', 'Туман'], correct: 1, explanation: 'Море резко отступило!', titleEn: 'Tsunami', questionEn: 'Sign of impending tsunami?', optionsEn: ['Wind', 'Sea recedes', 'Rain', 'Fog'], explanationEn: 'Rapid sea withdrawal — move to high ground!', d: 2 },
        { title: 'Наводнение', question: 'Опасная глубина воды?', image: 'droplet', options: ['5 см', '15 см', '10 см', '50 см'], correct: 1, explanation: '15 см может сбить с ног.', titleEn: 'Flooding', questionEn: 'Dangerous depth of flood water?', optionsEn: ['5 cm', '15 cm', '10 cm', '50 cm'], explanationEn: '15 cm can knock you off your feet.', d: 2 },
        { title: 'Active shooter', question: 'При стрелке в здании?', image: 'target', options: ['Атаковать', 'RUN-HIDE-FIGHT', 'Звонить', 'Кричать'], correct: 1, explanation: 'RUN → HIDE → FIGHT.', titleEn: 'Active Shooter', questionEn: 'During active shooter in building?', optionsEn: ['Attack', 'RUN-HIDE-FIGHT', 'Call only', 'Scream'], explanationEn: 'RUN → HIDE → FIGHT (last resort).', d: 2 },
        { title: 'Gas leak', question: 'При запахе газа?', image: 'fire', options: ['Позвонить', 'НЕ свет, покинуть', 'Открыть газ', 'Курить'], correct: 1, explanation: 'Не включать ничего!', titleEn: 'Gas Leak', questionEn: 'Upon smelling gas?', optionsEn: ['Call only', 'No switches — leave immediately', 'Open gas', 'Smoke'], explanationEn: 'Do not turn on/off anything — evacuate!', d: 2 },
        { title: 'ICS', question: 'Что такое ICS?', image: 'gauge', options: ['Интернет', 'Incident Command System', 'Камеры', 'Сирена'], correct: 1, explanation: 'Система управления инцидентами.', titleEn: 'ICS', questionEn: 'What is ICS?', optionsEn: ['Internet', 'Incident Command System', 'Cameras', 'Siren'], explanationEn: 'Incident Command System.', d: 3 },
        { title: 'Мародёрство', question: 'Защита после катастрофы?', image: 'building2', options: ['Уйти', 'Периметр+свет+патруль', 'Ждать', 'Плакат'], correct: 1, explanation: 'Периметр, освещение, охрана.', titleEn: 'Looting', questionEn: 'Protection after disaster?', optionsEn: ['Leave', 'Perimeter+lighting+patrol', 'Wait', 'Signs'], explanationEn: 'Perimeter, lighting, security patrol.', d: 3 }
      ]
    },
    {
      id: 'traffic',
      ...
      levels: [
        { title: 'ДТП', question: 'Первое действие при ДТП?', image: 'car', options: ['Фото', 'Безопасность', 'Друзьям', 'Уехать'], correct: 1, explanation: 'Безопасность!', titleEn: 'RTA', questionEn: 'First action at road traffic accident?', optionsEn: ['Take photos', 'Safety', 'Call friends', 'Leave'], explanationEn: 'Safety first!', d: 1 },
        { title: 'Пешеход', question: 'Безопасность пешехода ночью?', image: 'footprints', options: ['Тёмная одежда', 'Светоотражатели', 'Наушники', 'Бежать'], correct: 1, explanation: 'Светоотражатели!', titleEn: 'Pedestrian', questionEn: 'Pedestrian safety at night?', optionsEn: ['Dark clothing', 'Reflectors', 'Headphones', 'Run'], explanationEn: 'Reflective gear!', d: 1 },
        { title: 'Велосипедист', question: 'Защита велосипедиста?', image: 'bikeIcon', options: ['Ничего', 'Шлем', 'Бронежилет', 'Очки'], correct: 1, explanation: 'Шлем снижает риск на 85%.', titleEn: 'Cyclist', questionEn: 'Cyclist head protection?', optionsEn: ['Nothing', 'Helmet', 'Vest', 'Goggles'], explanationEn: 'Helmet reduces head injury risk by ~85%.', d: 1 },
        { title: 'Дистанция', question: 'Правило 3-х секунд?', image: 'target', options: ['Реакция', 'Безопасная дистанция', 'Обгон', 'Остановка'], correct: 1, explanation: '3 секунды до машины впереди.', titleEn: 'Following Distance', questionEn: 'What is the 3-second rule?', optionsEn: ['Reaction time', 'Safe following distance', 'Passing', 'Stopping'], explanationEn: 'Maintain 3 seconds behind vehicle ahead.', d: 2 },
        { title: 'Знак', question: 'Знак на трассе на расстоянии?', image: 'alertTriangle', options: ['10 м', '30 м', '100+ м', '5 м'], correct: 2, explanation: '100 метров минимум.', titleEn: 'Warning Sign', questionEn: 'Warning sign placement on highway?', optionsEn: ['10 m', '30 m', '100+ m', '5 m'], explanationEn: '100 meters minimum.', d: 2 },
        { title: 'Golden hour', question: '"Золотой час"?', image: 'clock', options: ['Час пик', '60 мин госпитализации', 'Закат', 'Обед'], correct: 1, explanation: 'Критические 60 минут.', titleEn: 'Golden Hour', questionEn: 'What is "golden hour"?', optionsEn: ['Rush hour', '60 min to hospital for trauma', 'Sunset', 'Lunch'], explanationEn: 'Critical 60 minutes for trauma care.', d: 2 },
        { title: 'Горящий авто', question: 'При пожаре машины бежать?', image: 'fire', options: ['К машине', '45В°, минимум 30м', 'В машину', 'Никуда'], correct: 1, explanation: 'Под углом 45В°, минимум 30м.', titleEn: 'Burning Vehicle', questionEn: 'When car is on fire, run?', optionsEn: ['Toward car', '45В° angle, min 30m', 'Into car', 'Nowhere'], explanationEn: '45В° angle, minimum 30m away.', d: 2 },
        { title: 'Извлечение', question: 'Когда извлекать из машины?', image: 'car', options: ['Всегда', 'При угрозе жизни', 'Никогда', 'Просьба'], correct: 1, explanation: 'Только при угрозе!', titleEn: 'Extrication', questionEn: 'When to extract from vehicle?', optionsEn: ['Always', 'Life-threatening situation', 'Never', 'On request'], explanationEn: 'Only when life is threatened!', d: 3 },
        { title: 'Регулировщик', question: 'Руки в стороны означают?', image: 'shieldCheck', options: ['Все едут', 'Стоп для всех', 'Прямо', 'Поворот'], correct: 1, explanation: 'Стоп для грудь/спина.', titleEn: 'Traffic Controller', questionEn: 'Arms outstretched mean?', optionsEn: ['All go', 'Stop for all', 'Straight', 'Turn'], explanationEn: 'Stop for traffic facing chest/back.', d: 3 },
        { title: 'Мотоцикл', question: 'Почему мотоциклистов не видят?', image: 'car', options: ['Быстрые', 'Маленький профиль', 'Без фар', 'Без звука'], correct: 1, explanation: 'Узкий профиль + слепые зоны.', titleEn: 'Motorcycle', questionEn: 'Why are motorcyclists not seen?', optionsEn: ['Too fast', 'Small profile', 'No headlights', 'No sound'], explanationEn: 'Narrow profile + blind spots.', d: 3 }
      ]
    }
  ];
  ...
  const MainMenu = () => (
    <div class="space-y-4 animate-fade-in">
      ...
      <div class="glass rounded-3xl p-5">
        <div class="flex items-center justify-between mb-4">
          <div>
            ...
            <p class="text-3xl font-bold bg-gradient-to-r from-sky-600 to-blue-600 bg-clip-text text-transparent">
        ...
        <div class="h-2 bg-gray-200 rounded-full overflow-hidden">
          <div 
            class="h-full bg-gradient-to-r from-sky-600 to-blue-700 rounded-full transition-all duration-500"
      ...
      <div class="flex items-center gap-2 mb-1">
        <Icon name="graduationCap" size="sm" class="text-sky-500" />
                ...
                <div class="flex items-center gap-3 relative">
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <div class="flex items-center gap-1.5 mt-1">
                      ...
                      <span class="text-[10px] px-1.5 py-0.5 bg-sky-100 text-sky-700 rounded-full">
                        ISO
                      </span>
                    </div>
                  </div>
      ...
      <div class="flex items-center gap-2 mb-1">
        <Icon name="shieldCheck" size="sm" class="text-sky-500" />
      ...
      <div class="glass rounded-2xl p-4 border border-sky-200 bg-sky-50/30">
        <div class="flex items-start gap-3">
          <Icon name="globe" class="text-sky-600" size="sm" />
          <div>
            <p class="font-medium text-sky-800">{t('academy.standards')}</p>
      ...
      <div class="animate-fade-in">
        ...
        <div class="glass rounded-3xl p-4 mb-4">
          <div class="flex items-center justify-between mb-3">
            ...
            <div class="text-right">
              ...
              <p class="font-bold text-sky-600">{score()}</p>
            </div>
          </div>
        ...
        <Show when={lv()}>
          <div class="glass rounded-3xl overflow-hidden mb-4">
            ...
            <div class="p-5">
              <div class="flex justify-center mb-4 animate-bounce-gentle"><SkillIcon icon={lv()!.image} size="xl" class="text-gray-700" /></div>
                  ...
                  {(option, i) => {
                    ...
                    const bgClass = () => {
                      if (revealed()) {
                        ...
                        return 'bg-sky-100 ring-2 ring-sky-500';
                      }
        ...
        <Show when={lives() === 0}>
          <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 animate-fade-in">
            <div class="glass rounded-3xl p-6 m-4 text-center animate-slide-up">
              <div class="flex justify-center mb-4"><Icon name="heart" size="xl" class="text-red-400" /></div>
              <p class="text-2xl font-bold text-gray-800 mb-2">Игра окончена</p>
              <p class="text-gray-600 mb-4">Ваш счёт: {score()}</p>
              <button 
                class="w-full py-3 bg-gradient-to-r from-sky-600 to-blue-700 text-white rounded-2xl font-bold touch-scale"
                onClick={() => { 
            ...
            return (
              ...
              >
                <div class="flex items-center justify-between mb-2">
                  ...
                  <Show when={isSelected()}>
                    <div class="w-6 h-6 rounded-full bg-gradient-to-br from-sky-600 to-blue-700 flex items-center justify-center">
                      <Icon name="check" class="text-white w-4 h-4" />
                    </div>
                  </Show>
                </div>
                <p class={`font-semibold ${isSelected() ? 'text-sky-700' : 'text-gray-800'}`}>
                  {lang.name}
                </p>
                <Show when={lang.rtl}>
      ...
      <div class="p-4">
        <div class="glass rounded-2xl p-4 flex items-start gap-3">
          <div class="w-8 h-8 rounded-full bg-sky-100 flex items-center justify-center flex-shrink-0">
            <Icon name="globe" class="text-sky-600" size="xs" />
          </div>
          <div>
            <p class="text-sm text-gray-600">
...
function RatingPage(props: { onBack: () => void }) {
  ...
  const worker = { name: 'Алексей К.', rating: 4.8, avatar: 'userCheck', profession };
  ...
  return (
    <div class="min-h-screen animate-fade-in">
      <Show when={!submitted()} fallback={
        <div class="min-h-screen flex flex-col items-center justify-center p-8 bg-gradient-to-b from-sky-600/20 to-transparent">
        ...
        <div class={`relative overflow-hidden rounded-b-3xl pb-8 pt-4 px-4 ${isDark() ? 'bg-gray-900' : ''}`}>
          ...
          <div class="relative mt-6 flex items-center gap-4">
            <div class="w-16 h-16 rounded-2xl bg-white/30 backdrop-blur flex items-center justify-center shadow-lg">
              <Icon name="user" size="lg" class="text-white" />
            </div>
            <div>
...
// ============== Auth Page ==============
const COUNTRY_CODES = [
  { code: 'KZ', flag: 'рџ‡°рџ‡ї', dial: '+7' },
  { code: 'RU', flag: 'рџ‡·рџ‡є', dial: '+7' },
  { code: 'FR', flag: 'рџ‡«рџ‡·', dial: '+33' },
  { code: 'US', flag: 'рџ‡єрџ‡ё', dial: '+1' },
  { code: 'DE', flag: 'рџ‡©рџ‡Є', dial: '+49' },
  { code: 'GB', flag: 'рџ‡¬рџ‡§', dial: '+44' },
  { code: 'CN', flag: 'рџ‡Ёрџ‡і', dial: '+86' },
  { code: 'TR', flag: 'рџ‡№рџ‡·', dial: '+90' },
];
        ...
        <Match when={step() === 1}>
          <div class="flex-1 flex flex-col p-6 pt-12">
            <div class="text-center mb-10">
              <h1 class="text-3xl font-bold bg-gradient-to-r from-sky-500 via-blue-500 to-pink-500 bg-clip-text text-transparent">
                BOLH
              </h1>
            ...
            <div class="flex-1">
              ...
              <div class="flex gap-2">
                ...
                <input
                  ...
                  class="flex-1 px-4 py-4 rounded-2xl glass text-gray-800 placeholder-gray-400 outline-none focus:ring-2 focus:ring-sky-500"
                  value={phone()}
            ...
            <button
              ...
              class="w-full py-4 rounded-2xl bg-gradient-to-r from-sky-600 to-blue-700 text-white font-bold text-lg shadow-lg touch-scale"
            >
              {t('auth.continue')}
            </button>
          ...
          <div class="flex-1 flex flex-col p-6 pt-12">
            ...
            <p class="text-center text-white/70 text-sm">
              {resendTimer() > 0 ? `Resend in 0:${String(resendTimer()).padStart(2, '0')}` : (
                <button type="button" class="text-sky-400 font-medium" onClick={() => setResendTimer(30)}>
                  {t('auth.resend')}
                </button>
              )}
            </p>
        ...
        <Match when={step() === 3}>
          <div class="flex-1 flex flex-col p-6 pt-12">
            ...
            <input
              type="text"
              placeholder="Alex"
              class="w-full px-4 py-4 rounded-2xl glass text-gray-800 placeholder-gray-400 mb-6 outline-none focus:ring-2 focus:ring-sky-500"
              value={name()}
            ...
            <div class="flex gap-3 mb-8">
              <button
                type="button"
                class={`flex-1 py-4 rounded-2xl font-medium touch-scale transition-all ${!isWorker() ? 'bg-gradient-to-r from-sky-600 to-blue-700 text-white shadow-lg' : 'glass text-gray-600'}`}
            ...
            <button
              ...
              class="w-full py-4 rounded-2xl bg-gradient-to-r from-sky-600 to-blue-700 text-white font-bold text-lg shadow-lg touch-scale mt-auto"
            >
              {t('auth.getStarted')}
            </button>
          </div>
    ...
    <div class="p-4 animate-fade-in">
      ...
      <div class="glass rounded-3xl p-6 mb-4 text-center animate-slide-up">
        <div class="relative inline-block mb-4">
          <div class="w-24 h-24 rounded-full bg-gradient-to-br from-sky-600 to-blue-700 flex items-center justify-center text-4xl text-white font-bold shadow-xl">
            AM
          </div>
        ...
        {/* Статус — маленький индикатор */}
        <div class="mt-3 flex items-center justify-center gap-2">
          <span class={`w-2.5 h-2.5 rounded-full ${statusColor()}`} />
          <span class={`text-sm font-medium ${
            workerStatus() === 'online' ? 'text-green-600' :
            workerStatus() === 'busy' ? 'text-amber-600' : 'text-gray-400'
          }`}>{statusText()}</span>
        </div>
      ...
      {/* Online / Offline — главная кнопка */}
      <button
        type="button"
        class={`w-full mb-4 p-4 rounded-3xl flex items-center justify-center gap-3 touch-scale transition-all animate-slide-up ${
          workerStatus() === 'online'
        ...
        onClick={() => {
          if (workerStatus() === 'online') {
            boardSetAllActive(false);
            setWorkerStatus('offline');
            haptic('light');
          } else {
            boardSetAllActive(true);
            setWorkerStatus('online');
            haptic('heavy');
          }
        }}
      >
        <div class={`w-4 h-4 rounded-full transition-all ${workerStatus() === 'online' ? 'bg-green-400 shadow-lg shadow-green-400/50' : 'bg-white/20'}`} />
        <span class={`text-sm font-bold ${workerStatus() === 'online' ? 'text-green-400' : 'text-white/40'}`}>
          {workerStatus() === 'online'
            ? (currentLang() === 'en' ? 'Online — accepting orders' : 'Онлайн — принимаю заказы')
            : (currentLang() === 'en' ? 'Go Online' : 'Выйти в сеть')
          }
        </span>
        <Show when={workerStatus() === 'online'}>
          <span class="text-green-400/60 text-xs ml-auto">{boardItems().filter(b => b.active).length} {currentLang() === 'en' ? 'skills' : 'навыков'}</span>
        </Show>
      </button>

      {/* Мои навыки — компактная сводка + кнопка Изменить */}
      <div class="glass rounded-3xl p-4 mb-4 animate-slide-up" style="animation-delay: 0.05s">
        <div class="flex items-center justify-between mb-3">
          <h3 class="text-white font-bold text-sm">{currentLang() === 'en' ? 'My Skills' : 'Мои навыки'}</h3>
          <button
            type="button"
            class="px-3 py-1.5 rounded-xl bg-sky-500/20 text-sky-400 text-xs font-bold touch-scale"
        ...
        <div class="flex items-center gap-4 mb-3">
          <div class="flex items-center gap-2">
            <span class="text-2xl font-bold text-white">{totalActiveSkills()}</span>
            <span class="text-white/50 text-xs">{currentLang() === 'en' ? 'skills' : 'навыков'}</span>
          </div>
          <div class="w-px h-6 bg-white/10" />
          <div class="flex items-center gap-2">
            <span class="text-2xl font-bold text-white">{activeDeptCount()}</span>
            <span class="text-white/50 text-xs">{currentLang() === 'en' ? 'depts' : 'отделов'}</span>
          </div>
        </div>
        {/* Активные отделы — маленькие бейджи */}
        <div class="flex flex-wrap gap-1.5">
          <For each={departments.filter(d => isDeptActive(d.id))}>
            {(dept) => (
              <span
                class="text-[10px] font-semibold px-2 py-1 rounded-lg text-white/90"
                style={`background: linear-gradient(135deg, ${dept.colorFrom}30, ${dept.colorTo}20); border: 1px solid ${dept.colorFrom}25;`}
              >
                {currentLang() === 'en' ? dept.nameEn : dept.name} ({deptSkillCount(dept.id)})
              </span>
            )}
          </For>
          <Show when={activeDeptCount() === 0}>
            <span class="text-white/30 text-xs">{currentLang() === 'en' ? 'No skills selected yet' : 'Навыки не выбраны'}</span>
          </Show>
        </div>
      </div>
      ...
      <Show when={showDiplomaPrompt()}>
        <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-[200] p-6" onClick={() => setShowDiplomaPrompt(null)}>
          <div class={`glass rounded-3xl p-6 max-w-sm w-full animate-slide-up ${isDark() ? 'bg-gray-800' : ''}`} onClick={(e) => e.stopPropagation()}>
            ...
            <button
              type="button"
              class="w-full py-3 bg-gradient-to-r from-sky-500 to-sky-600 text-white rounded-2xl font-semibold mb-3 touch-scale flex items-center justify-center gap-2"
            ...
            return (
              <button 
                class={`w-full p-4 flex items-center gap-4 touch-scale border-b border-gray-100 last:border-0 ${
                  ...
                  (item as any).highlight ? 'bg-gradient-to-r from-sky-50/50 to-blue-50/50' : ''
                }`}
              ...
              >
                ...
                <Icon name={isSpecial ? 'play' : 'chevronRight'} class={isSpecial ? 'text-sky-500' : 'text-slate-400 dark:text-slate-500'} size="sm" />
              </button>
            );
          }}
        </For>
...
function SecurityCenterPage(props: { onBack: () => void }) {
  ...
  const dataCards = () => [
    ...
    { id: 'pay', icon: 'creditCard' as const, title: 'Payment protection', desc: 'All payments processed securely', color: 'from-violet-500 to-blue-600' },
  ...
  return (
    <div class="h-screen flex flex-col animate-fade-in bg-gradient-to-b from-slate-900/20 to-transparent">
      ...
      <div class={`flex items-center gap-3 p-4 safe-area-top ${isDark() ? 'bg-gray-900/80' : 'bg-white/80'} backdrop-blur-xl border-b border-gray-200/50 shadow-sm`}>
        ...
        <div class="w-11 h-11 rounded-full bg-gradient-to-br from-sky-600 to-blue-700 flex items-center justify-center text-white font-bold shadow-lg flex-shrink-0">
          АК
        </div>
        <div class="flex-1 min-w-0">
      ...
      <div class={`p-3 safe-area-bottom ${isDark() ? 'bg-gray-900/90' : 'bg-white/90'} backdrop-blur-xl border-t border-gray-200/50`}>
        <div class="flex items-center gap-2 glass rounded-2xl pl-4 pr-2 py-2">
          ...
          <button
            class="w-10 h-10 rounded-xl bg-gradient-to-br from-sky-600 to-blue-700 flex items-center justify-center shadow-lg touch-scale disabled:opacity-50"
  ...
  return (
    <div class="min-h-screen animate-fade-in pb-8">
      ...
      <div class={`flex items-center justify-between p-4 safe-area-top ${isDark() ? 'bg-gray-900/90' : 'bg-white/90'} backdrop-blur-xl border-b border-gray-200/50`}>
        ...
        <button class="text-sm font-medium text-sky-600 touch-scale">{t('notifications.markRead')}</button>
      </div>
            ...
            {(n) => (
              <div
                class={`glass rounded-2xl p-4 flex gap-3 touch-scale border-l-4 ${
                  n.unread ? 'border-sky-500' : 'border-transparent'
                } ${isDark() ? 'bg-white/5' : ''}`}
              >
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-start justify-between gap-2">
                    ...
                    <Show when={n.unread}>
                      <span class="w-2.5 h-2.5 rounded-full bg-sky-500 flex-shrink-0 mt-1.5" />
                    </Show>
                  </div>
...
function SkillDetailPage(props: { onBack: () => void }) {
  ...
  return (
    <div class="min-h-screen animate-fade-in">
      ...
      <div class={`bg-gradient-to-br ${dept()?.color || 'from-sky-600 to-blue-700'} px-4 pt-3 pb-5`} style="padding-top: max(env(safe-area-inset-top), 12px)">
        <div class="flex items-center gap-3 mb-4">
          ...
          {(group, gi) => {
            ...
            return (
              <div class="mb-5 animate-slide-up" style={`animation-delay: ${gi() * 0.05}s`}>
                {/* Заголовок группы */}
                <div class="flex items-center gap-3 mb-3">
                  <div class={`w-14 h-14 rounded-2xl bg-gradient-to-br ${dept()?.color || 'from-sky-600 to-blue-700'} flex items-center justify-center shadow-lg`}>
      ...
      <Show when={localDiplomaPrompt()}>
        <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-[200] p-6" onClick={() => setLocalDiplomaPrompt(null)}>
          <div class={`glass rounded-3xl p-6 max-w-sm w-full animate-slide-up ${isDark() ? 'bg-gray-800' : ''}`} onClick={(e) => e.stopPropagation()}>
            ...
            <button type="button" class="w-full py-3 bg-gradient-to-r from-blue-500 to-sky-600 text-white rounded-2xl font-semibold mb-3 touch-scale flex items-center justify-center gap-2"
  ...
  return (
    <div class="min-h-screen animate-fade-in">
      ...
      <div class="p-4 space-y-4">
        ...
        <div class="glass rounded-3xl p-5 animate-slide-up">
          <div class="flex items-center gap-3 mb-4">
            <div class="w-14 h-14 rounded-2xl bg-gradient-to-br from-violet-500 to-blue-600 flex items-center justify-center shadow-lg">
              <Icon name="volume2" size="lg" class="text-sky-500" />
            </div>
            <div class="flex-1">
          ...
          <div class="grid grid-cols-4 gap-2">
            {(['tap', 'success', 'error', 'notify'] as const).map(snd => (
              ...
              >
                <span class="block mb-1 flex justify-center">
                  <Icon name={snd === 'tap' ? 'zap' : snd === 'success' ? 'checkCircle' : snd === 'error' ? 'alertCircle' : 'bell'} size="sm" class="text-gray-700" />
                </span>
          ...
          <div class="grid grid-cols-4 gap-2 mt-2">
            {(['send', 'receive', 'delete', 'levelup'] as const).map(snd => (
              ...
              >
                <span class="block mb-1 flex justify-center">
                  <Icon name={snd === 'send' ? 'send' : snd === 'receive' ? 'download' : snd === 'delete' ? 'trash' : 'star'} size="sm" class="text-gray-700" />
                </span>
                <span class="text-[10px] font-medium text-gray-600 capitalize">{snd}</span>
              </button>
            ))}
          ...
          {(section, idx) => (
            <div class="glass rounded-3xl overflow-hidden animate-slide-up" style={`animation-delay: ${0.05 * (idx() + 1)}s`}>
              <div class="px-5 py-3 flex items-center gap-2">
                <Icon name={section.icon} size="sm" class="text-gray-600" />
                    ...
                    <Show when={item.type === 'slider'}>
                      <div class="w-24 flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                        <input
                          ...
                          class="w-full accent-sky-500 h-1.5"
                        />
                      </div>
                    </Show>
        ...
        <div class="glass rounded-3xl p-5 text-center animate-slide-up" style="animation-delay: 0.25s">
          <div class="w-16 h-16 rounded-2xl bg-gradient-to-br from-sky-600 to-blue-700 flex items-center justify-center mx-auto mb-3 shadow-lg">
...
const navItems: { id: string; icon: keyof typeof Icons; labelKey: string; center?: boolean }[] = [
      ...
      <main class={['tracking', 'map'].includes(currentPage()) ? 'h-screen' : showNav() ? 'pb-24' : 'pb-4'}>
        <Switch>
          ...
          <Match when={currentPage() === 'myboard'}>
            {/* ══════════ PREMIUM MY BOARD ══════════ */}
            {(() => {
              const [boardMode, setBoardMode] = createSignal<'worker' | 'client'>('worker');
              const [longPressId, setLongPressId] = createSignal<string | null>(null);
              const [onlinePulse, setOnlinePulse] = createSignal(false);
              let lpTimer: any = null;

              const allSkillsFlat = () => departments.flatMap(d => d.skills.map(s => ({ ...s, deptId: d.id, cf: d.colorFrom, ct: d.colorTo })));
              const homeSkillIds = () => boardMode() === 'worker' ? workerSkills() : clientNeeds();
              const boardForMode = () => boardItems().filter(b => b.role === boardMode());
              const activeForMode = () => boardForMode().filter(b => b.active).length;
              const totalForMode = () => homeSkillIds().length;
              const isOnline = () => workerStatus() === 'online';

              // Group skills by department — only selected ones
              const groupedSkills = () => {
                const selected = homeSkillIds();
                const boardMap = new Map(boardForMode().map(b => [b.id, b]));
                const groups: { dept: typeof departments[0]; skills: (typeof allSkillsFlat extends () => (infer T)[] ? (T & { boardItem: any }) : never)[] }[] = [];
                for (const dept of departments) {
                  const deptSkills = dept.skills.filter(s => selected.includes(s.id)).map(s => ({
                    ...s, deptId: dept.id, cf: dept.colorFrom, ct: dept.colorTo,
                    boardItem: boardMap.get(s.id) || null,
                  }));
                  if (deptSkills.length > 0) groups.push({ dept, skills: deptSkills as any });
                }
                return groups;
              };

              const myCourses = () => boardItems().filter(b => b.role === 'course');
              const [academyOpen, setAcademyOpen] = createSignal(false);
              // Compact academy modules list for Board
              const academyModules: { id: string; lucide: keyof typeof Icons; name: string; nameEn: string; color: string; levels: number }[] = [
                { id: 'pro_plumbing', lucide: 'wrench', name: 'Сантехника', nameEn: 'Plumbing', color: '#3b82f6', levels: 12 },
                { id: 'pro_electrical', lucide: 'zap', name: 'Электрика', nameEn: 'Electrical', color: '#f59e0b', levels: 12 },
                { id: 'pro_locks', lucide: 'keyRound', name: 'Замки', nameEn: 'Locks', color: '#64748b', levels: 10 },
                { id: 'pro_tech', lucide: 'monitor', name: 'Техника', nameEn: 'Tech Repair', color: '#8b5cf6', levels: 8 },
                { id: 'pro_home', lucide: 'home', name: 'Дом и ремонт', nameEn: 'Home Repair', color: '#10b981', levels: 10 },
                { id: 'pro_cleaning', lucide: 'sparkles', name: 'Клининг', nameEn: 'Cleaning', color: '#06b6d4', levels: 8 },
                { id: 'pro_moving', lucide: 'truck', name: 'Грузоперевозки', nameEn: 'Moving', color: '#f97316', levels: 10 },
                { id: 'pro_security', lucide: 'shield', name: 'Охрана', nameEn: 'Security', color: '#ef4444', levels: 10 },
                { id: 'pro_auto', lucide: 'car', name: 'Авто', nameEn: 'Auto', color: '#0ea5e9', levels: 10 },
                { id: 'fire', lucide: 'fire', name: 'Пожарная', nameEn: 'Fire Safety', color: '#dc2626', levels: 10 },
                { id: 'first_aid', lucide: 'heart', name: 'Первая помощь', nameEn: 'First Aid', color: '#22c55e', levels: 10 },
                { id: 'hazmat', lucide: 'alertTriangle', name: 'Хим. безопасность', nameEn: 'Hazmat', color: '#eab308', levels: 8 },
              ];

              const handleLPS = (id: string) => { lpTimer = setTimeout(() => { setLongPressId(id); haptic('medium'); }, 500); };
              const handleLPE = () => { if (lpTimer) { clearTimeout(lpTimer); lpTimer = null; } };

              const goOnline = () => { boardSetAllActive(true); setWorkerStatus('online'); setOnlinePulse(true); haptic('heavy'); setTimeout(() => setOnlinePulse(false), 1200); };
              const goOffline = () => { boardSetAllActive(false); setWorkerStatus('offline'); haptic('light'); };
              const pct = () => totalForMode() > 0 ? Math.round((activeForMode() / totalForMode()) * 100) : 0;

              return (
                <div class="relative min-h-screen animate-fade-in" onClick={() => longPressId() && setLongPressId(null)}>
                  {/* ── Wallpaper ── */}
                  <div class="absolute inset-0 overflow-hidden pointer-events-none select-none" style="z-index:0;">
                    <div class="grid grid-cols-7 gap-0.5 p-1 opacity-[0.04]">
                      <For each={allSkillsFlat()}>
                        {(s) => <div class="flex items-center justify-center aspect-square"><SkillIcon icon={s.icon} class="text-white" size="sm" /></div>}
                      </For>
                    </div>
                    {/* Gradient fade over wallpaper */}
                    <div class="absolute inset-0" style="background: linear-gradient(180deg, rgba(5,5,7,0.95) 0%, rgba(5,5,7,0.5) 30%, rgba(5,5,7,0.5) 70%, rgba(5,5,7,0.95) 100%);" />
                  </div>
                  ...
                  <div class="relative p-4" style="z-index:1;">
                      ...
                      <div class="relative w-14 h-14 shrink-0">
                        <svg viewBox="0 0 36 36" class="w-full h-full -rotate-90">
                          ...
                          <circle cx="18" cy="18" r="15.5" fill="none"
                            stroke={isOnline() ? '#4ade80' : '#38bdf8'}
                            stroke-width="3" stroke-linecap="round"
                            stroke-dasharray={`${pct() * 0.975} 97.5`}
                            class="transition-all duration-700"
                          />
                        </svg>
                        <div class="absolute inset-0 flex items-center justify-center">
                          <span class="text-white text-xs font-bold">{activeForMode()}</span>
                        </div>
                      </div>
                      <div class="flex-1">
                        <h1 class="text-lg font-bold text-white leading-tight">{currentLang() === 'en' ? 'My Board' : 'Моя панель'}</h1>
                        <p class="text-white/35 text-[11px]">
                          {totalForMode()} {currentLang() === 'en' ? 'skills' : 'навыков'} В· {activeForMode()} {currentLang() === 'en' ? 'active' : 'активных'}
                        </p>
                      </div>
                      {/* Quick add — go to home */}
                      <button
                        type="button"
                        class="w-10 h-10 rounded-2xl flex items-center justify-center touch-scale"
                        style="background: rgba(99,102,241,0.15); border: 1px solid rgba(99,102,241,0.25);"
                        onClick={() => { setHomeMode(boardMode() === 'worker' ? 'order' : 'search'); setCurrentPage('home'); haptic('light'); }}
                      >
                        <Icon name="plus" size="sm" class="text-sky-400" />
                      </button>
                    </div>

                    {/* ▸ Mode toggle — pill style */}
                    <div class="flex rounded-2xl p-1 mb-4" style="background: rgba(255,255,255,0.06);">
                      <button
                        type="button"
                        class={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5 ${
                          boardMode() === 'client'
                            ? 'bg-white text-gray-800 shadow-lg shadow-white/10'
                            : 'text-white/40'
                        }`}
                        onClick={() => setBoardMode('client')}
                      >
                        <Icon name="search" size="xs" /> {currentLang() === 'en' ? 'Find a Pro' : 'Ищу мастера'}
                      </button>
                      <button
                        type="button"
                        class={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5 ${
                          boardMode() === 'worker'
                            ? 'bg-gradient-to-r from-sky-600 to-blue-700 text-white shadow-lg shadow-sky-500/20'
                            : 'text-white/40'
                        }`}
                        onClick={() => setBoardMode('worker')}
                      >
                        <Icon name="wrench" size="xs" class="inline" /> {currentLang() === 'en' ? 'I Work' : 'Я мастер'}
                      </button>
                    </div>

                    {/* ▸ Online card — worker mode only */}
                    <Show when={boardMode() === 'worker'}>
                      <div
                        class={`rounded-3xl p-4 mb-4 touch-scale cursor-pointer transition-all ${onlinePulse() ? 'animate-pulse' : ''}`}
                        style={isOnline()
                          ? 'background: linear-gradient(135deg, rgba(34,197,94,0.15), rgba(16,185,129,0.08)); border: 1.5px solid rgba(34,197,94,0.25); box-shadow: 0 0 30px rgba(34,197,94,0.08);'
                          : 'background: rgba(255,255,255,0.03); border: 1.5px solid rgba(255,255,255,0.06);'
                        }
                        onClick={() => isOnline() ? goOffline() : goOnline()}
                      >
                        <div class="flex items-center gap-3">
                          <div class={`w-10 h-10 rounded-2xl flex items-center justify-center ${isOnline() ? '' : 'opacity-40'}`}
                            style={isOnline()
                              ? 'background: linear-gradient(135deg, #22c55e, #10b981);'
                              : 'background: rgba(255,255,255,0.06);'
                            }
                          >
                            <div class={`w-3.5 h-3.5 rounded-full ${isOnline() ? 'bg-white shadow-lg' : 'bg-white/20'}`} />
                          </div>
                          <div class="flex-1">
                            <p class={`text-sm font-bold ${isOnline() ? 'text-green-300' : 'text-white/40'}`}>
                              {isOnline() ? (currentLang() === 'en' ? 'Online' : 'Онлайн') : (currentLang() === 'en' ? 'Go Online' : 'Выйти в сеть')}
                            </p>
                            <p class={`text-[10px] ${isOnline() ? 'text-green-400/50' : 'text-white/20'}`}>
                              {isOnline()
                                ? (currentLang() === 'en' ? `Searching... ${activeForMode()} skills active` : `Ищем... ${activeForMode()} навыков активно`)
                                : (currentLang() === 'en' ? 'Tap to start accepting orders' : 'Нажми чтобы принимать заказы')
                              }
                            </p>
                          </div>
                          <div class={`w-12 h-7 rounded-full p-0.5 transition-all ${isOnline() ? 'bg-green-500' : 'bg-white/10'}`}>
                            <div class={`w-6 h-6 rounded-full bg-white shadow transition-all ${isOnline() ? 'translate-x-5' : 'translate-x-0'}`} />
                          </div>
                        </div>
                      </div>
                    </Show>
                    ...
                    <Show when={groupedSkills().length > 0}>
                          ...
                          <div class="mb-4">
                            ...
                            <div class="flex items-center gap-2 mb-2 px-1">
                              <div class="w-5 h-5 rounded-md flex items-center justify-center"
                                style={`background: linear-gradient(135deg, ${group.dept.colorFrom}40, ${group.dept.colorTo}25);`}
                              >
                                <SkillIcon icon={group.dept.icon} class="text-white" size="xs" />
                              </div>
                              <p class="text-white/40 text-[10px] font-bold uppercase tracking-wider flex-1">{currentLang() === 'en' ? group.dept.nameEn : group.dept.name}</p>
                              <span class="text-white/20 text-[10px]">{group.skills.filter((s: any) => s.boardItem?.active).length}/{group.skills.length}</span>
                            </div>
                            {/* Skills grid */}
                            <div class="grid grid-cols-4 gap-2">
                              <For each={group.skills}>
                                {(item: any) => {
                                  const isActive = () => item.boardItem?.active ?? true;
                                  const isLP = () => longPressId() === item.id;

                                  return (
                                    <div
                                      class="relative rounded-2xl p-1.5 flex flex-col items-center text-center transition-all cursor-pointer select-none"
                                      style={isActive()
                                        ? `background: linear-gradient(145deg, ${group.dept.colorFrom}10, ${group.dept.colorTo}06); border: 1.5px solid ${group.dept.colorFrom}22;`
                                        : 'background: rgba(255,255,255,0.02); border: 1.5px solid rgba(255,255,255,0.05);'
                                      }
                                      onClick={() => { if (!longPressId()) { boardToggleActive(item.id); haptic('light'); } }}
                                      onTouchStart={() => handleLPS(item.id)}
                                      onTouchEnd={handleLPE}
                                      onTouchMove={handleLPE}
                                    >
                                      <div class={`w-10 h-10 rounded-xl flex items-center justify-center mb-0.5 transition-all ${isActive() ? '' : 'opacity-30 grayscale'}`}
                                        style={isActive()
                                          ? `background: linear-gradient(135deg, ${group.dept.colorFrom}25, ${group.dept.colorTo}15);`
                                          : 'background: rgba(255,255,255,0.04);'
                                        }
                                      >
                                        <SkillIcon icon={item.icon} class="text-white" size="sm" />
                                      </div>
                                      <p class={`text-[7px] leading-tight font-semibold max-w-full truncate ${isActive() ? 'text-white/80' : 'text-white/20'}`}>{currentLang() === 'en' ? item.nameEn : item.name}</p>
                                      <Show when={isActive()}>
                                        <div class="absolute top-0.5 right-0.5 w-2 h-2 rounded-full" style={`background: ${group.dept.colorFrom}; box-shadow: 0 0 6px ${group.dept.colorFrom}60;`} />
                                      </Show>
                                      <Show when={isLP()}>
                                        <div class="absolute inset-0 bg-black/70 rounded-2xl flex items-center justify-center animate-fade-in">
                                          <button type="button" class="w-8 h-8 rounded-full bg-red-500 flex items-center justify-center text-white text-xs font-bold shadow-lg touch-scale"
                                            onClick={(e) => {
                                              e.stopPropagation();
                                              if (boardMode() === 'worker') setWorkerSkills(prev => prev.filter(s => s !== item.id));
                                              else setClientNeeds(prev => prev.filter(s => s !== item.id));
                                              boardRemove(item.id);
                                              setLongPressId(null);
                                              haptic('medium');
                                            }}
                    ...
                    {/* ▸ BOLH Academy — collapsible */}
                    <div class="rounded-3xl mb-4 overflow-hidden" style="background: linear-gradient(145deg, rgba(168,85,247,0.06), rgba(99,102,241,0.03)); border: 1px solid rgba(168,85,247,0.12);">
                      {/* Academy header — tap to expand */}
                      <button
                        type="button"
                        class="w-full p-3 flex items-center gap-3 touch-scale"
                        onClick={() => { setAcademyOpen(a => !a); haptic('light'); }}
                      >
                        <div class="w-11 h-11 rounded-2xl flex items-center justify-center shrink-0" style="background: linear-gradient(135deg, rgba(168,85,247,0.25), rgba(99,102,241,0.15));">
                          <Icon name="graduationCap" size="sm" class="text-sky-300" />
                        </div>
                        <div class="flex-1 text-left">
                          <p class="text-white/80 text-sm font-bold">BOLH {currentLang() === 'en' ? 'Academy' : 'Академия'}</p>
                          <p class="text-white/25 text-[10px]">{academyModules.length} {currentLang() === 'en' ? 'courses В· Safety & Pro skills' : 'курсов В· Безопасность и навыки'}</p>
                        </div>
                        <div class={`w-6 h-6 rounded-lg flex items-center justify-center transition-transform ${academyOpen() ? 'rotate-180' : ''}`} style="background: rgba(168,85,247,0.1);">
                          <span class="text-blue-400/50 text-xs">▼</span>
                        </div>
                      </button>

                      {/* Expanded course list */}
                      <Show when={academyOpen()}>
                        <div class="px-3 pb-3 animate-fade-in">
                          <div class="border-t border-blue-500/10 pt-3">
                            <div class="grid grid-cols-3 gap-2">
                              <For each={academyModules}>
                                {(mod) => (
                                  <button
                                    type="button"
                                    class="rounded-2xl p-2 flex flex-col items-center text-center touch-scale transition-all"
                                    style={`background: ${mod.color}08; border: 1.5px solid ${mod.color}18;`}
                                    onClick={() => { setCurrentPage('academy'); haptic('light'); }}
                                  >
                                    <div class="w-10 h-10 rounded-xl flex items-center justify-center mb-1" style={`background: ${mod.color}18;`}>
                                      <span class="text-lg">{mod.icon}</span>
                                    </div>
                                    <p class="text-white/60 text-[8px] font-semibold leading-tight">{currentLang() === 'en' ? mod.nameEn : mod.name}</p>
                                    <p class="text-white/20 text-[7px] mt-0.5">{mod.levels} {currentLang() === 'en' ? 'lvl' : 'ур.'}</p>
                                  </button>
                                )}
                              </For>
                            </div>
                    ...
                    {/* ▸ Empty state */}
                    <Show when={totalForMode() === 0}>
                      <div class="rounded-3xl p-8 text-center" style="background: rgba(255,255,255,0.03); border: 1.5px dashed rgba(255,255,255,0.08);">
                        <div class="w-16 h-16 rounded-3xl flex items-center justify-center mx-auto mb-4" style="background: linear-gradient(135deg, rgba(99,102,241,0.15), rgba(168,85,247,0.1));">
                          <Icon name={boardMode() === 'worker' ? 'wrench' : 'search'} size="lg" class="text-sky-400" />
                        </div>
                        <p class="text-white/60 text-sm font-semibold mb-1">
                          {boardMode() === 'worker'
                            ? (currentLang() === 'en' ? 'No skills yet' : 'Навыков ещё нет')
                            : (currentLang() === 'en' ? 'No services yet' : 'Услуг ещё нет')
                          }
                        </p>
                        <p class="text-white/25 text-xs mb-5">
                          {currentLang() === 'en' ? 'Select on Home — they appear here automatically' : 'Выбери на Главной — появятся тут сами'}
                        </p>
                        <button type="button" class="px-5 py-2.5 rounded-2xl text-sm font-bold touch-scale"
                          style="background: linear-gradient(135deg, rgba(99,102,241,0.2), rgba(168,85,247,0.15)); color: #38bdf8; border: 1px solid rgba(99,102,241,0.25);"
                          onClick={() => { setHomeMode(boardMode() === 'worker' ? 'order' : 'search'); setCurrentPage('home'); }}
                        >
                          {currentLang() === 'en' ? 'Go to Home' : 'Перейти на главную'}
                        </button>
                      </div>
                    </Show>

                    {/* ▸ Bottom hint */}
                    <p class="text-white/10 text-[8px] text-center mt-5 tracking-wider uppercase">
                      {currentLang() === 'en' ? 'auto-sync В· long press to remove В· tap to toggle' : 'авто-синхронизация В· удержи = убрать В· тап = вкл/выкл'}
                    </p>
                  </div>
                </div>
              );
                ...
                return isCenter() ? (
                  ...
                  >
                    <div class={`w-[52px] h-[52px] rounded-2xl flex items-center justify-center transition-all duration-300 ${
                      ...
                      <Icon
                        name={item.icon}
                        class={isActive() ? 'text-black' : 'text-white/50'}
                      />
                    </div>
...
function WalletPage(props: { onBack: () => void; onNavigate?: (page: string) => void }) {
  ...
  return (
    <div class="min-h-screen animate-fade-in">
      {/* Gradient header */}
      <div class="bg-gradient-to-br from-sky-600 via-blue-600 to-violet-700 px-4 pt-3 pb-4" style="padding-top: max(env(safe-area-inset-top), 12px)">
        <div class="flex items-center gap-3 mb-3">
          <button type="button" class="w-10 h-10 rounded-2xl bg-white/20 flex items-center justify-center touch-press backdrop-blur-sm"
            onClick={() => { playGlobalSound('swoosh'); props.onBack(); }}>
            <Icon name="chevronLeft" class="text-white" size="sm" />
      ...
      <Show when={!loading()} fallback={<div class="flex items-center justify-center py-12"><div class="w-8 h-8 border-3 border-sky-500 border-t-transparent rounded-full animate-spin" /></div>}>
        ...
        <Show when={activeTab() === 'balance'}>
          <Show when={wallet()} fallback={
            ...
            <div class="flex flex-col items-center justify-center py-12">
              <div class="w-20 h-20 rounded-full bg-gradient-to-br from-sky-100 to-blue-100 flex items-center justify-center mb-4">
                <Icon name="lock" size="lg" class="text-sky-500" />
              </div>
              ...
              <button
                class="px-8 py-3 rounded-2xl bg-gradient-to-r from-sky-600 to-blue-700 text-white font-bold text-sm touch-scale active:scale-95"
            ...
            <div class="relative rounded-3xl overflow-hidden mb-5" style="background: linear-gradient(135deg, #0ea5e9, #8b5cf6, #a78bfa)">
                ...
                <div class={`w-full max-w-md rounded-t-3xl p-6 animate-slide-up ${isDark() ? 'bg-gray-900' : 'bg-white'}`}>
                  ...
                  <div class="space-y-4">
                    <div>
                      ...
                      <input
                        class={`w-full px-4 py-3 rounded-xl text-sm font-mono focus:outline-none focus:border-sky-400 focus:ring-2 focus:ring-sky-100 ${isDark() ? 'bg-gray-800 border border-gray-700 text-gray-100' : 'bg-gray-50 border border-gray-200 text-gray-800'}`}
                        placeholder="bolh1..."
                        value={sendTo()}
                    ...
                    <button
                      class="w-full py-3.5 rounded-2xl bg-gradient-to-r from-sky-600 to-blue-700 text-white font-bold text-sm touch-scale disabled:opacity-50"
            ...
            <div class={`rounded-2xl overflow-hidden ${isDark() ? 'bg-gray-800/60 border border-gray-700/50' : 'glass'}`}>
              <div class="px-4 py-3 flex items-center justify-between">
                ...
                <span class="text-xs text-sky-500 font-medium">Blockchain</span>
              </div>
              <For each={txHistory()}>
                {(tx: any) => {
        ...
        <Show when={activeTab() === 'chain'}>
          ...
          <div class="rounded-3xl overflow-hidden mb-5" style="background: linear-gradient(135deg, #0f172a, #1e293b)">
            <div class="p-5">
              <div class="flex items-center gap-3 mb-4">
                <div class="w-12 h-12 rounded-full bg-gradient-to-br from-sky-600 to-blue-700 flex items-center justify-center">
              ...
              <div class="mt-4 pt-4 border-t border-white/10">
                ...
                <div class="space-y-2">
                  {[
                    { label: 'Mining / Earn', pct: 60, color: 'from-sky-500 to-blue-500', amount: '6B' },
                    { label: 'Реферальная программа', pct: 20, color: 'from-blue-500 to-pink-500', amount: '2B' },
            ...
            <div class="grid grid-cols-2 gap-3 text-xs">
              <div class="flex items-center gap-2">
                <div class="w-6 h-6 rounded-lg bg-sky-100 flex items-center justify-center">
                  <Icon name="shield" size="xs" class="text-sky-600" />
                </div>
                <div>
          ...
          <div class={`rounded-2xl p-4 mb-5 ${isDark() ? 'bg-gray-800/60 border border-gray-700/50' : 'glass'}`}>
            ...
            <div class="grid grid-cols-2 gap-3">
              {([
                ...
                { icon: 'eye' as const, title: currentLang() === 'en' ? 'Expert' : 'Эксперт', desc: currentLang() === 'en' ? 'Proxy missions' : 'Прокси-миссии', color: 'from-blue-500 to-fuchsia-500', iconBg: 'bg-slate-100 dark:bg-slate-800/60', iconColor: 'text-slate-600 dark:text-slate-300' },
              ] as const).map(mod_ => (
          ...
          <div class="space-y-3">
            <button class={`w-full rounded-2xl p-4 flex items-center gap-4 touch-scale text-left ${isDark() ? 'bg-gray-800/60 border border-gray-700/50' : 'glass'}`} onClick={() => props.onNavigate?.('referral')}>
              <div class="w-12 h-12 rounded-2xl bg-gradient-to-br from-sky-100 to-blue-100 flex items-center justify-center">
                <Icon name="handshake" size="sm" class="text-sky-600" />
              </div>
              <div class="flex-1">
            ...
            <button class={`w-full rounded-2xl p-4 flex items-center gap-4 touch-scale text-left ${isDark() ? 'bg-gray-800/60 border border-gray-700/50' : 'glass'}`}
              ...
              <div class="w-12 h-12 rounded-2xl bg-gradient-to-br from-sky-100 to-blue-100 flex items-center justify-center">
                <Icon name="fileText" size="sm" class="text-sky-600" />
              </div>
              <div class="flex-1">
            ...
            <Show when={contractsOpen()}>
              <div class={`rounded-2xl p-4 ${isDark() ? 'bg-gray-800/80 border border-gray-700/50' : 'bg-white/90 border border-gray-200/60'} space-y-4 animate-fade-in`}>
                ...
                <div class="grid grid-cols-4 gap-2">
                  {([
                    { v: contractStats()?.total_created ?? 0, l: currentLang() === 'en' ? 'Total' : 'Всего', icon: 'fileText' as const, iconColor: 'text-sky-500' },
                    { v: contractStats()?.active_count ?? 0, l: currentLang() === 'en' ? 'Active' : 'Активных', icon: 'circleDot' as const, iconColor: 'text-sky-500 dark:text-sky-400' },
                ...
                <button
                  class="w-full py-3 rounded-xl bg-gradient-to-r from-sky-600 to-blue-700 text-white font-bold text-sm active:scale-[0.97] transition-transform flex items-center justify-center gap-2"
                ...
                <Show when={escrowOpen()}>
                  <div class={`rounded-xl p-4 space-y-3 ${isDark() ? 'bg-gray-700/60' : 'bg-sky-50/80'}`}>
                    <div class={`text-xs font-bold ${isDark() ? 'text-sky-300' : 'text-sky-700'}`}>
                    ...
                    <input
                      ...
                      class={`w-full px-3 py-2.5 rounded-xl text-sm ${isDark() ? 'bg-gray-800 text-white border-gray-600 placeholder:text-gray-500' : 'bg-white text-gray-800 border-gray-200 placeholder:text-gray-400'} border outline-none focus:ring-2 focus:ring-sky-500/30`}
                    ...
                    <button
                      class="w-full py-3 rounded-xl bg-gradient-to-r from-sky-600 to-blue-700 text-white font-bold text-sm active:scale-[0.97] transition-transform disabled:opacity-50"
                        ...
                        return (
                          <div class={`rounded-xl p-3 ${isDark() ? 'bg-gray-700/50 border border-gray-600/30' : 'bg-gray-50 border border-gray-200/50'}`}>
                            ...
                            <Show when={c.state === 'Active' && !isClient}>
                              <button
                                class="w-full py-2 rounded-lg bg-sky-500/20 text-sky-400 text-xs font-bold active:scale-[0.97] transition disabled:opacity-50"
                ...
                <div class={`rounded-xl p-3 ${isDark() ? 'bg-gray-700/40' : 'bg-gray-50'}`}>
                  <div class={`text-[10px] space-y-2 ${isDark() ? 'text-gray-400' : 'text-gray-500'}`}>
                    <div class="flex items-start gap-1.5"><Icon name="shieldCheck" size="xs" class="text-sky-500 mt-px shrink-0" /> <span><b>Escrow</b> — {currentLang() === 'en' ? 'Funds locked until service confirmed by both' : 'Средства заблокированы до подтверждения обеими сторонами'}</span></div>
        ...
        <Show when={activeTab() === 'network'}>
          ...
          <div class={`rounded-2xl overflow-hidden mb-5 ${isDark() ? 'bg-gray-800/60 border border-gray-700/50' : 'glass'}`}>
            ...
            <For each={p2pPeers()}>
              {(peer: any) => (
                <div class={`px-4 py-3 border-t flex items-center gap-3 ${isDark() ? 'border-gray-700/50' : 'border-gray-100'}`}>
                  <div class="w-8 h-8 rounded-full bg-emerald-100 flex items-center justify-center text-sm"><Icon name="monitor" size="xs" class="text-emerald-600" /></div>
                  <div class="flex-1 min-w-0">
            ...
            <Show when={p2pPeers().length === 0}>
              <div class="px-4 py-8 text-center">
                <div class="flex justify-center mb-2"><Icon name="wifi" size="lg" class="text-gray-400" /></div>
        ...
        <Show when={activeTab() === 'explorer'}>
          <Show when={explorerLoading()}>
            <div class="flex items-center justify-center py-12"><div class="w-8 h-8 border-3 border-sky-500 border-t-transparent rounded-full animate-spin" /></div>
          </Show>
          <Show when={!explorerLoading()}>
            {/* Chain Overview Hero */}
            <div class="rounded-3xl overflow-hidden mb-5" style="background: linear-gradient(135deg, #1e1b4b, #312e81)">
              <div class="p-5">
                <div class="flex items-center gap-3 mb-4">
                  <div class="w-12 h-12 rounded-full bg-sky-400/20 flex items-center justify-center">
                    <Icon name="search" size="lg" class="text-sky-300" />
                  </div>
                  <div>
                    <div class="text-white font-bold text-lg">BOLH Explorer</div>
                    <div class="text-sky-300 text-xs font-medium">
                ...
                <div class="grid grid-cols-3 gap-3 mb-3">
                  <div class="bg-white/10 rounded-xl p-3 text-center">
                    ...
                    <div class="text-sky-200 text-[10px]">{currentLang() === 'en' ? 'Blocks' : 'Блоков'}</div>
                  </div>
                  <div class="bg-white/10 rounded-xl p-3 text-center">
                    <div class="text-white font-bold text-xl">{explorerData()?.chain?.total_transactions ?? 0}</div>
                    <div class="text-sky-200 text-[10px]">{currentLang() === 'en' ? 'Transactions' : 'Транзакций'}</div>
                  </div>
                  <div class="bg-white/10 rounded-xl p-3 text-center">
                    <div class="text-white font-bold text-xl">{explorerData()?.chain?.total_accounts ?? 0}</div>
                    <div class="text-sky-200 text-[10px]">{currentLang() === 'en' ? 'Accounts' : 'Аккаунтов'}</div>
                  </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-3">
                  <div class="bg-white/10 rounded-xl p-3">
                    <div class="flex items-center gap-2 mb-1">
                      <Icon name="fileText" size="xs" class="text-sky-300" />
                      <span class="text-sky-200 text-[10px]">{currentLang() === 'en' ? 'Smart Contracts' : 'Контракты'}</span>
                    </div>
                    <div class="text-white font-bold">{explorerData()?.contracts?.total_created ?? 0}</div>
                  </div>
                  <div class="bg-white/10 rounded-xl p-3">
                    <div class="flex items-center gap-2 mb-1">
                      <Icon name="lock" size="xs" class="text-sky-300" />
                      <span class="text-sky-200 text-[10px]">{currentLang() === 'en' ? 'Value Locked' : 'Залочено'}</span>
                    </div>
            ...
            <div class={`rounded-2xl p-4 mb-5 ${isDark() ? 'bg-gray-800/60 border border-gray-700/50' : 'glass'}`}>
              ...
              <div class={`font-mono text-[10px] break-all p-2 rounded-lg ${isDark() ? 'bg-gray-700/50 text-sky-300' : 'bg-gray-100 text-sky-600'}`}>
            ...
            <div class={`rounded-2xl p-4 mb-5 ${isDark() ? 'bg-gray-800/60 border border-gray-700/50' : 'glass'}`}>
              <div class="flex items-center justify-between mb-3">
                ...
                <button class="text-xs text-sky-500 font-bold active:scale-95" onClick={loadExplorer}>
                ...
                <For each={explorerBlocks()}>
                  {(block) => (
                    ...
                    >
                      <div class="flex items-center justify-between mb-1">
                        <div class="flex items-center gap-2">
                          <div class={`w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold ${block.height === 0 ? 'bg-yellow-100 text-yellow-700' : 'bg-sky-100 text-sky-700'}`}>
            ...
            <Show when={selectedBlock()}>
              <div class={`rounded-2xl p-4 mb-5 ${isDark() ? 'bg-sky-900/30 border border-sky-700/50' : 'bg-sky-50 border border-sky-200'}`}>
                <div class="flex items-center justify-between mb-3">
                  <h3 class={`text-sm font-bold ${isDark() ? 'text-sky-300' : 'text-sky-700'}`}>
                ...
                <div class="space-y-2 text-xs">
                  {[
                    ...
                    <div class={`flex justify-between py-1.5 border-b ${isDark() ? 'border-sky-700/30' : 'border-sky-200/50'}`}>
                      <span class={isDark() ? 'text-sky-400' : 'text-sky-500'}>{row.label}</span>
                      <span class={`font-mono text-[10px] ${isDark() ? 'text-sky-200' : 'text-sky-700'}`}>{row.value}</span>
                    </div>
                  ))}
                </div>
                {/* Block Transactions */}
                <Show when={selectedBlock()?.transactions?.length > 0}>
                  <div class="mt-3 pt-3 border-t border-sky-700/30">
                    <div class={`text-xs font-bold mb-2 ${isDark() ? 'text-sky-300' : 'text-sky-700'}`}>
                      {currentLang() === 'en' ? 'Transactions' : 'Транзакции'}
                    </div>
                    <div class="space-y-1.5">
                      <For each={selectedBlock()?.transactions ?? []}>
                        {(tx: any) => (
                          <div class={`rounded-lg p-2 ${isDark() ? 'bg-sky-800/30' : 'bg-sky-100/60'}`}>
                            <div class="flex items-center justify-between">
                              <span class={`text-[10px] font-medium ${isDark() ? 'text-sky-300' : 'text-sky-600'}`}>{tx.tx_type}</span>
                              <span class={`text-[10px] font-bold ${isDark() ? 'text-white' : 'text-sky-800'}`}>{BC.formatBOLH(tx.amount)} BOLH</span>
                            </div>
                            <div class={`text-[9px] font-mono mt-0.5 ${isDark() ? 'text-sky-400/70' : 'text-sky-500/70'}`}>
            ...
            <div class={`rounded-2xl p-4 ${isDark() ? 'bg-gray-800/60 border border-gray-700/50' : 'glass'}`}>
              ...
              <div class="space-y-2">
                {([
                  ...
                  { icon: 'fileText' as const, title: 'Smart Contracts', desc: currentLang() === 'en' ? 'Escrow, Bounty, Subscription, Insurance' : 'Escrow, Bounty, Подписки, Страхование', color: 'text-sky-500', bg: 'bg-sky-100' },
...
function ReferralPage(props: { onBack: () => void }) {
  ...
  const tiersList = [
    ...
    { id: 4, label: 'Tier 4', range: '100 001+', reward: '500', color: 'from-sky-400 to-sky-500', icon: 'target' as const, iconColor: 'text-sky-500 dark:text-sky-400' },
  ];
  ...
  return (
    <div class="min-h-screen animate-fade-in">
      {/* Gradient header */}
      <div class="bg-gradient-to-br from-sky-500 via-blue-500 to-fuchsia-500 px-4 pt-3 pb-5" style="padding-top: max(env(safe-area-inset-top), 12px)">
        <div class="flex items-center gap-3 mb-4">
          <button type="button" class="w-10 h-10 rounded-2xl bg-white/20 flex items-center justify-center touch-press backdrop-blur-sm"
            onClick={() => { playGlobalSound('swoosh'); props.onBack(); }}>
            <Icon name="chevronLeft" class="text-white" size="sm" />
        ...
        <Show when={!loading()}>
          ...
          <div class={`${cardBg()} rounded-2xl p-4 mb-4`}>
            ...
            <div class="flex items-center gap-2 mb-3">
              ...
              <Show when={myCode()}>
                <button
                  class={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all ${copied() ? 'bg-green-500 text-white' : 'bg-sky-500 text-white'}`}
                  onClick={copyCode}
                >
            ...
            <p class={`text-xs mb-4 ${txtSub()}`}>
              {currentLang() === 'en' ? 'Invite a friend — you' : 'Пригласи друга — вы'} <span class={`font-bold ${txt()}`}>{currentLang() === 'en' ? 'both' : 'оба'}</span> {currentLang() === 'en' ? 'get' : 'получите'} <span class="font-bold text-sky-500 text-base">{rewardPerPerson()} BOLH</span>
            </p>
            <div class="flex gap-2">
              <button
                class="flex-1 py-3 rounded-2xl bg-gradient-to-r from-sky-600 to-blue-700 text-white font-bold text-sm active:scale-[0.98] transition-all flex items-center justify-center gap-2 shadow-lg touch-scale"
                onClick={shareLink}
              >
            ...
            {([
              ...
              <button
                class={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all ${
                  activeTab() === tab.key
                    ? (isDark() ? 'bg-gray-700 text-sky-400 shadow-sm' : 'bg-white text-sky-600 shadow-sm')
          ...
          <Show when={activeTab() === 'enter'}>
            <div class={`${cardBg()} rounded-2xl p-5 mb-4`}>
              ...
              <div class="flex gap-2">
                ...
                />
                <button
                  class="px-5 py-3 rounded-xl bg-sky-500 text-white font-bold text-sm active:scale-[0.97] transition-all disabled:opacity-50"
          ...
          <Show when={activeTab() === 'overview'}>
            ...
            <div class={`${cardBg()} rounded-2xl p-5 mb-4`}>
              ...
              <div class="space-y-4">
                {[
                  { n: '1', bg: 'bg-sky-100 dark:bg-sky-900/40', tc: 'text-sky-600', title: currentLang() === 'en' ? 'Share your code' : 'Поделись кодом', desc: currentLang() === 'en' ? 'Send your code or link to a friend' : 'Отправь код или ссылку другу' },
            ...
            <div class={`${cardBg()} rounded-2xl p-5 mb-4`}>
              ...
              <div class={`w-full h-3 rounded-full overflow-hidden mb-3 ${isDark() ? 'bg-gray-700' : 'bg-gray-100'}`}>
                <div class="h-full rounded-full bg-gradient-to-r from-sky-500 to-blue-500 transition-all" style={`width: ${Math.min(parseFloat(poolPercent()), 100)}%`} />
              </div>
            ...
            <div class={`${cardBg()} rounded-2xl p-5`}>
              ...
              <p class={`text-xs mb-3 ${txtSub()}`}>{currentLang() === 'en' ? 'Reward' : 'Награда'}: <span class="font-bold text-sky-500">{rewardPerPerson()} BOLH</span> {currentLang() === 'en' ? 'each' : 'каждому'}</p>
              <div class="flex items-center gap-2">
                ...
                <For each={programStats()?.top_referrers || []}>
                  {(r: any, i) => (
                    <div class={`flex items-center gap-3 py-2 border-b last:border-0 ${isDark() ? 'border-gray-700/50' : 'border-gray-100'}`}>
                      <div class="w-7 h-7 rounded-full bg-gradient-to-br from-sky-400 to-blue-500 flex items-center justify-center text-white text-xs font-bold">{i() + 1}</div>
                      <div class="flex-1 min-w-0">
                        <div class={`text-xs font-mono truncate ${isDark() ? 'text-gray-300' : 'text-gray-600'}`}>{r.code}</div>
                      </div>
                      <div class="text-right">
                        <div class="text-sm font-bold text-sky-500">{r.referral_count}</div>
          ...
          <Show when={activeTab() === 'friends'}>
            <Show when={referralHistory().length > 0} fallback={
              <div class={`${cardBg()} rounded-2xl p-8 text-center`}>
                ...
                <button class="px-5 py-2.5 rounded-xl bg-sky-500 text-white font-bold text-sm touch-scale" onClick={shareLink}>
                ...
                <For each={referralHistory()}>
                  {(f: any) => (
                    <div class={`px-4 py-3 border-b last:border-0 flex items-center gap-3 ${isDark() ? 'border-gray-700/30' : 'border-gray-50'}`}>
                      <div class="w-9 h-9 rounded-full bg-gradient-to-br from-sky-400 to-blue-500 flex items-center justify-center flex-shrink-0">
            ...
            <button class="w-full mt-4 py-3.5 rounded-2xl bg-gradient-to-r from-sky-600 to-blue-700 text-white font-bold active:scale-[0.98] transition-all flex items-center justify-center gap-2 shadow-lg touch-scale" onClick={shareLink}>
              <Icon name="plus" class="text-white" size="sm" />
          ...
          <Show when={activeTab() === 'tiers'}>
            <div class="space-y-3">
              <For each={tiersList}>
                {(tier) => (
                  <div class={`rounded-2xl overflow-hidden ${tier.id === currentTier() ? (isDark() ? 'ring-2 ring-sky-400 ring-offset-2 ring-offset-gray-900' : 'ring-2 ring-sky-500 ring-offset-2') : 'shadow-sm'}`}>
  ...
  return (
    <div class="p-4 animate-fade-in pb-8">
      ...
      <div class="space-y-3 mb-6">
        <For each={plans}>{(plan) => (
          <div class={`glass rounded-2xl p-4 relative ${plan.recommended ? 'border-2 border-sky-500 shadow-lg' : ''}`}>
            <Show when={plan.recommended}><span class="absolute -top-2.5 right-4 px-3 py-0.5 bg-sky-500 text-white text-xs font-bold rounded-full">{t('payments.recommended')}</span></Show>
            <div class="flex items-center justify-between mb-2">
              <div><h3 class="font-bold text-gray-800">{plan.name}</h3><p class="text-xl font-bold text-sky-600">{plan.price > 0 ? `${plan.price.toLocaleString()} ₸` : t('payments.free')}<span class="text-sm text-gray-500 font-normal">/{t('payments.month')}</span></p></div>
              <Show when={plan.current} fallback={<button class="px-4 py-2 bg-sky-500 text-white rounded-xl text-sm font-medium touch-scale" onClick={() => { haptic('medium'); playGlobalSound('success'); }}>{t('payments.select')}</button>}>
      ...
      <div class="glass rounded-2xl overflow-hidden mb-6">
        ...
        <button class="w-full p-4 text-sky-500 font-medium text-sm flex items-center justify-center gap-2 touch-scale"><Icon name="plus" size="sm" class="text-sky-500" />{t('payments.addCard')}</button>
      </div>
...
function AchievementsPage(props: { onBack: () => void }) {
  ...
  const rc: Record<string,string> = { common: 'from-gray-400 to-gray-500', rare: 'from-blue-400 to-blue-600', epic: 'from-blue-500 to-pink-500', legendary: 'from-amber-400 to-orange-500' };
  return (
    <div class="p-4 animate-fade-in pb-8">
      ...
      <div class="bg-gradient-to-br from-sky-600 to-blue-700 rounded-3xl p-6 text-white text-center mb-5 shadow-xl">
      ...
      <div class="grid grid-cols-3 gap-3 mb-5">
        <div class="glass rounded-2xl p-3 text-center"><p class="text-2xl font-bold text-sky-600">{achs.filter(a=>a.unlocked).length}</p><p class="text-xs text-gray-500">{t('achievements.unlocked')}</p></div>
      ...
      <div class="space-y-3"><For each={achs}>{(a) => (
        <div class={`glass rounded-2xl p-4 flex items-center gap-4 ${a.unlocked ? '' : 'opacity-60'}`}>
          ...
          <div class="flex-1">
            ...
            <Show when={!a.unlocked && a.progress > 0}><div class="mt-2"><div class="h-1.5 bg-gray-200 rounded-full overflow-hidden"><div class="h-full bg-sky-500 rounded-full" style={`width:${(a.progress/a.max)*100}%`} /></div><p class="text-xs text-gray-400 mt-1">{a.progress}/{a.max}</p></div></Show>
          </div>
  ...
  return (
    <div class="p-4 animate-fade-in pb-8">
      ...
      <div class="glass rounded-2xl p-4 mb-5">
        ...
        <div class="flex items-end justify-between h-28 gap-2"><For each={weekly}>{(d)=>(<div class="flex-1 flex flex-col items-center"><div class="w-full bg-gradient-to-t from-sky-500 to-blue-500 rounded-t" style={`height:${(d.amt/maxE)*100}%;min-height:4px`}/><p class="text-[10px] text-gray-500 mt-1.5">{d.day}</p></div>)}</For></div>
...
function MarketplacePage(props: { onBack: () => void }) {
  ...
  return (
    <div class="p-4 animate-fade-in pb-8">
      ...
      <div class="flex gap-2 overflow-x-auto pb-3 -mx-4 px-4 mb-4"><For each={cats}>{(c)=>(<button onClick={()=>setActiveCat(c)} class={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap touch-scale ${activeCat()===c?'bg-sky-500 text-white':'glass text-gray-600'}`}>{t('marketplace.cat.'+c)}</button>)}</For></div>
      <div class="grid grid-cols-2 gap-3"><For each={filtered()}>{(p)=>(
        <div class="glass rounded-2xl overflow-hidden">
          ...
          <div class="p-3">
            ...
            <div class="flex items-center gap-2"><span class="font-bold text-sky-600">{p.price.toLocaleString()} ₸</span><Show when={p.oldPrice}><span class="text-xs text-gray-400 line-through">{p.oldPrice?.toLocaleString()}</span></Show></div>
            <button class={`w-full mt-2 py-2 rounded-xl text-sm font-medium touch-scale ${p.inStock?'bg-sky-500 text-white':'bg-gray-200 text-gray-400'}`} disabled={!p.inStock} onClick={()=>{if(p.inStock){setCartCount(cartCount()+1);haptic('light');playGlobalSound('success');}}}>{p.inStock?t('marketplace.addToCart'):t('marketplace.outOfStock')}</button>
          </div>
        </div>
      )}</For></div>
    </div>
...
function CreateOrderPage(props: { onBack: () => void }) {
  ...
  return (
    <div class="p-4 animate-fade-in pb-24">
      ...
      <div class="grid grid-cols-2 gap-3 mb-5"><For each={services}>{(s)=>(<button onClick={()=>{setSel(s.id);haptic('light');playGlobalSound('tap');}} class={`p-4 rounded-2xl text-left touch-scale ${sel()===s.id?'glass border-2 border-sky-400 shadow-md':'glass'}`}><Icon name={s.icon} class={sel()===s.id?'text-sky-500':'text-gray-400'} size="lg"/><p class="font-medium text-gray-800 mt-2">{t(s.label)}</p><p class="text-sm text-sky-600">{s.price.toLocaleString()} ₸/{t('order.hour')}</p></button>)}</For></div>
      ...
      <div class="glass rounded-2xl p-4 mb-6">
        ...
        <div class="flex justify-between"><span class="font-bold text-gray-900">{t('order.total')}</span><span class="text-xl font-bold text-sky-600">{total().toLocaleString()} ₸</span></div>
      </div>
      <div class="fixed bottom-0 left-0 right-0 p-4 safe-area-bottom bg-white/80 backdrop-blur-lg" style="z-index:50">
        <button class="w-full py-4 bg-gradient-to-r from-sky-600 to-blue-700 text-white rounded-2xl font-bold text-lg touch-scale disabled:opacity-50 flex items-center justify-center gap-2" disabled={!sel()||!addr()||loading()} onClick={submit}>
...
function WorkerDetailPage(props: { onBack: () => void; onNavigate: (page: string) => void }) {
  ...
  return (
    <div class="animate-fade-in pb-24">
      <div class="bg-gradient-to-br from-sky-600 to-blue-700 px-4 pt-4 pb-20"><button onClick={props.onBack} class="flex items-center gap-2 text-white/80 touch-scale mb-2"><Icon name="chevronLeft" class="text-white" size="sm"/><span class="text-sm">{t('nav.back')}</span></button></div>
      <div class="glass rounded-3xl -mt-16 mx-4 p-5 text-center relative">
        <div class="w-20 h-20 rounded-full bg-gradient-to-br from-sky-400 to-blue-500 flex items-center justify-center text-3xl text-white font-bold shadow-xl mx-auto -mt-14 border-4 border-white">А</div>
        ...
        <div class="flex border-t border-gray-100 divide-x divide-gray-100 mt-4">
          ...
          <div class="flex-1 py-3 text-center"><p class="text-lg font-bold text-sky-600">{w.rate.toLocaleString()} ₸</p><p class="text-xs text-gray-500">{t('worker.perHour')}</p></div>
        </div>
      </div>
      <div class="mx-4 mt-4 glass rounded-2xl p-4"><p class="font-semibold text-gray-800 mb-2">{t('worker.about')}</p><p class="text-gray-600 text-sm">{w.bio}</p><div class="flex flex-wrap gap-2 mt-3"><For each={w.specs}>{(s)=>(<span class="px-3 py-1 bg-sky-100 text-sky-700 text-xs rounded-full font-medium">{s}</span>)}</For></div></div>
      ...
      <div class="fixed bottom-0 left-0 right-0 p-4 safe-area-bottom bg-white/80 backdrop-blur-lg" style="z-index:50">
        <div class="flex gap-3">
          ...
          <button class="flex-1 py-3 bg-gradient-to-r from-sky-600 to-blue-700 text-white rounded-2xl font-bold flex items-center justify-center gap-2 touch-scale" onClick={()=>props.onNavigate('createorder')}><Icon name="zap" size="sm" class="text-white"/>{t('worker.bookNow')}</button>
        </div>
      </div>
    </div>
  );
