...
export default function DocumentDetailPage({ params }: { params: { id: string } }) {
  ...
  const handleDelete = async () => {
    if (!params.id || !confirm('Удалить документ?')) return
    try {
  ...
  const handleToggleSaveLocal = () => {
    ...
    try {
      ...
      setActionHint(next.includes(params.id) ? 'Сохранено в локальном хранилище' : 'Убрано из локального хранилища')
    } catch {
      // ignore
    }
  }
  ...
  const shareDocument = async () => {
    if (typeof window === 'undefined') return
    const payload = { title: doc?.title || 'Document', text: doc?.description || 'Document from BOLH', url: window.location.href }
    try {
      if (navigator.share) {
      ...
      setActionHint('Данные для отправки скопированы')
    } catch {
      // ignore
    }
  }
  ...
  const openInNewTab = async () => {
    ...
    try {
      ...
      setActionHint('Открыто в новой вкладке')
    } catch {
      // ignore
    }
  }
  ...
  const sendByEmail = async () => {
    if (typeof window === 'undefined') return
    const subject = encodeURIComponent(`Document: ${doc?.title || 'File'}`)
  ...
  const copyMetadata = async () => {
    ...
    try {
      ...
      setActionHint('Метаданные скопированы')
    } catch {
      // ignore
    }
  }
      ...
      <div className="min-h-screen bg-[#1a1b26] text-white pb-24 flex items-center justify-center">
        {!loading && !doc ? (
          <div className="text-center">
            <p className="text-white/60">Документ не найден</p>
            <Link href="/documents" className="text-violet-400 mt-2 inline-block">К списку</Link>
          </div>
        ) : (
      ...
      <main className="mx-auto max-w-lg px-4 py-6 space-y-6">
        <div className="rounded-2xl bg-white/10 p-6 flex flex-col items-center gap-3">
          ...
          {doc.status === 'signed' && (
            <span className="rounded bg-green-500/20 text-green-300 text-sm px-3 py-1">
              Подписан{doc.signed_by ? ` · ${formatDate(doc.signature_date)}` : ''}
            </span>
          )}
          {savedLocally && (
            <span className="rounded bg-violet-500/20 text-violet-200 text-xs px-2.5 py-1">
              Сохранён в избранном хранилище
            </span>
          )}
          {actionHint ? (
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="rounded-xl bg-white/5 p-3">
            <p className="text-xs text-white/50">Создан</p>
            <p className="text-sm">{formatDate(doc.created_at)}</p>
          </div>
          <div className="rounded-xl bg-white/5 p-3">
            <p className="text-xs text-white/50">Версия</p>
          ...
          <div className="rounded-xl bg-white/5 p-3">
            <p className="text-xs text-white/50">Последнее обновление</p>
        ...
        {doc.description && (
          <div>
            <p className="text-xs font-medium text-white/50 uppercase mb-1">Описание</p>
        ...
        {doc.status !== 'signed' && (
          <div className="rounded-xl bg-white/10 p-4 space-y-2">
            <p className="text-sm font-medium">Электронная подпись</p>
            <input
              type="text"
              value={signText}
        ...
        <div className="rounded-xl bg-white/5 border border-white/10 p-3 text-xs text-white/70 space-y-2">
          <p>Быстрые действия с файлом</p>
          <div className="flex flex-wrap gap-2">
            <button type="button" onClick={sendByEmail} className="rounded-lg bg-white/10 px-3 py-1.5 flex items-center gap-1.5">
              <Mail className="h-3.5 w-3.5" />
              Отправить по email
            </button>
            {lastDownloadedAt && <span className="rounded-lg bg-white/10 px-3 py-1.5">Последнее скачивание: {formatDate(lastDownloadedAt)}</span>}
            ...
            {doc.expires_at ? <span className="rounded-lg bg-white/10 px-3 py-1.5">Истекает: {formatDate(doc.expires_at)}</span> : null}
            <span className="rounded-lg bg-white/10 px-3 py-1.5">Горячие клавиши: D скачать, C копировать ссылку</span>
          </div>
        </div>
      </main>
