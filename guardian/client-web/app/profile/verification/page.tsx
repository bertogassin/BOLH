...
export default function VerificationPage() {
  ...
  const loadStatus = async () => {
    ...
    try {
      ...
      setStatus(null)
      setError(e instanceof Error ? e.message : 'Не удалось загрузить статус верификации')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSubmit = async () => {
    if (!docBase64) {
      setError('Сначала выберите документ для отправки')
      return
    }
    setSubmitting(true)
    setError('')
    ...
    try {
      ...
      setSuccess('Документ отправлен. Проверка начата.')
      setDocBase64('')
      await loadStatus()
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Не удалось отправить документы')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  const badge = (() => {
    if (status?.verified) return { text: 'Подтверждено', cls: 'bg-green-500/20 text-green-200 border-green-500/40', icon: CheckCircle2 }
    if (status?.requested && status?.status === 'pending') return { text: 'На проверке', cls: 'bg-amber-500/20 text-amber-200 border-amber-500/40', icon: Clock3 }
    if (status?.requested && status?.status === 'rejected') return { text: 'Отклонено', cls: 'bg-red-500/20 text-red-200 border-red-500/40', icon: AlertTriangle }
    return { text: 'Не начато', cls: 'bg-violet-500/20 text-violet-200 border-violet-500/40', icon: ShieldCheck }
  })()
        ...
        <div className="space-y-4">
            ...
            <p className="text-white/80 text-center">
              {status?.verified
                ...
                : status?.requested && status?.status === 'pending'
                  ? 'Проверка документов в процессе.'
                  : status?.requested && status?.status === 'rejected'
                    ? 'Верификация отклонена. Загрузите новый документ и отправьте повторно.'
                    : 'Подтвердите личность для доступа к большему числу заказов.'}
            </p>
            <button
              type="button"
              onClick={loadStatus}
          ...
          {!status?.verified && (
            <div className="rounded-2xl bg-white/10 p-4 border border-violet-400 space-y-3">
              <p className="text-sm text-white/80">Загрузите документ (паспорт/ID/право на работу) для проверки:</p>
              <label className="flex items-center justify-center gap-2 rounded-xl border border-violet-400 bg-white/10 hover:bg-white/15 px-3 py-3 cursor-pointer min-h-[44px]">
                ...
                <span className="text-sm font-medium">{fileName ? 'Документ выбран' : 'Выбрать документ'}</span>
